...
public class FileDialog : DialogBase
{
	...
	public void show(FileDialogMode mode, string title, string extensions, Action<string> callback)
	{
		_clipboardPath = "";
		_mode = mode;
		...
		_filenameInput.text = DefaultFilename;
		DefaultDirectory = DefaultDirectory.Replace('/', Path.DirectorySeparatorChar);
		switch (mode)
		{
		case FileDialogMode.Load:
			_actionText.text = "Load";
			break;
		case FileDialogMode.Save:
			_actionText.text = "Save";
			break;
		case FileDialogMode.Path:
			_actionText.text = "Select";
			break;
		}
		_extensionsDropDown.ClearOptions();
		_extensionsDropDown.options.Add(new Dropdown.OptionData("Known"));
		string[] source = Extensions.Split(new char[1] { ';' }, StringSplitOptions.RemoveEmptyEntries);
		_extensionsDropDown.AddOptions(source.ToList());
		_extensionsDropDown.options.Add(new Dropdown.OptionData("All"));
		_extensionsDropDown.value = 0;
		LastDirectory = PlayerPrefs.GetString("FileDialog_LastDirectory");
		...
		_pathScrollRect.gameObject.SetActive(ShowPath);
		_placesScrollRect.gameObject.SetActive(ShowPlaces);
		_previewPanel.SetActive(ShowPreview);
		_filenameInput.gameObject.SetActive(ShowInputField);
		if (ShowPlaces)
		{
			updatePlaces();
		}
	...
	public void setPath(string path)
	{
		...
		else
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				text = text + text2 + Path.DirectorySeparatorChar;
	...
	private string getFullFileName(FileListItem item)
	{
		if (_path == "%%ROOT%%")
		{
			return item.Filename + Path.DirectorySeparatorChar;
		}
