...
public class ScreenBuffer
{
	...
	{
		ushort attributes = ColorUtility.MakeColor(f, b);
		if (_X < Width && _X >= 0 && _Y < Height && _Y >= 0)
		{
			this[_X, _Y].Char = (char)c;
			this[_X, _Y].Attributes = attributes;
		}
		if (X != Width - 1)
		{
			X++;
	...
	{
		ushort background = 0;
		ushort attributes = ColorUtility.MakeColor(7, background);
		if (_X < Width && _X >= 0 && _Y < Height && _Y >= 0)
		{
			this[_X, _Y].Char = (char)c;
			this[_X, _Y].Attributes = attributes;
	...
	public void Write(string Tile, string RenderString, string ColorString, string TileColor, char DetailColor, bool SuppressImposters = true, bool HFlip = false, bool VFlip = false)
	{
		...
		if (!string.IsNullOrEmpty(Tile) && Options.UseTiles)
		{
			...
			if (DetailColor == '\0')
			{
				consoleChar.Detail = ColorUtility.ColorMap['k'];
			}
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			consoleChar.Attributes = ColorUtility.MakeColor(value2, value3);
		}
		X++;
		if (X >= Width)
		{
	...
	{
		Color result = ColorUtility.ColorMap['y'];
	...
	{
		Color result = ColorUtility.ColorMap['k'];
		for (int i = 0; i < s.Length; i++)
		{
			if (s[i] != '^')
			{
	...
	public int WriteBlockWithNewlines(string s, int MaxLines = 999, int StartLine = 0, bool drawIndicators = false)
	{
		...
		ushort value = 7;
		ushort value2 = 0;
		ushort attributes = ColorUtility.MakeColor(value, value2);
		int x = _X;
		int num = 1;
		int num2 = 0;
		for (int i = 0; i < s.Length; i++)
		{
			if (s[i] == '&')
			{
				...
				else
				{
					...
					attributes = ColorUtility.MakeColor(value, value2);
				}
				continue;
			}
			if (s[i] == '^')
	...
	public ScreenBuffer Write(string s, bool processMarkup = true, bool HFlip = false, bool VFlip = false)
	{
		if (string.IsNullOrEmpty(s))
		{
			return this;
			...
			if (s[i] == '&')
			{
				...
				if (s[i] == '&')
				{
					if (_X >= 0 && _Y >= 0 && _X < Width && _X >= 0 && _Y < Height && _Y >= 0)
					{
						this[_X, _Y].Char = '&';
						this[_X, _Y].Attributes = attributes;
						this[_X, _Y].HFlip = HFlip;
						this[_X, _Y].VFlip = VFlip;
					}
					if (X == Width - 1)
					{
						return this;
	...
	public ScreenBuffer Write(StringBuilder s)
	{
		if (s.Length == 0)
		{
			return this;
	...
	public void BeveledBox(int x1, int y1, int x2, int y2, ushort ForeColor, ushort BackColor)
	{
		...
		{
			Buffer[i, y1].Char = 'Ü';
			Buffer[i, y1].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
			Buffer[i, y1].HFlip = false;
			Buffer[i, y1].VFlip = false;
			Buffer[i, y2].Char = 'Ü';
			Buffer[i, y2].Attributes = ColorUtility.MakeColor(ForeColor, BackColor);
		...
		{
			Buffer[x1, j].Char = 'Ý';
			Buffer[x1, j].Attributes = ColorUtility.MakeColor(ForeColor, BackColor);
			Buffer[x1, j].HFlip = false;
			Buffer[x1, j].VFlip = false;
			Buffer[x2, j].Char = 'Ý';
			Buffer[x2, j].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
			Buffer[x2, j].HFlip = false;
			Buffer[x2, j].VFlip = false;
		}
		Buffer[x1, y1].Char = 'Ú';
		Buffer[x1, y1].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
		Buffer[x1, y1].HFlip = false;
		Buffer[x1, y1].VFlip = false;
		Buffer[x1, y2].Char = 'À';
		Buffer[x1, y2].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
		Buffer[x1, y2].HFlip = false;
		Buffer[x1, y2].VFlip = false;
		Buffer[x2, y1].Char = '¿';
		Buffer[x2, y1].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
		Buffer[x2, y1].HFlip = false;
		Buffer[x2, y1].VFlip = false;
		Buffer[x2, y2].Char = 'Ù';
		Buffer[x2, y2].Attributes = ColorUtility.MakeColor(BackColor, ForeColor);
	...
	public void ThickSingleBox(int x1, int y1, int x2, int y2, ushort Color)
	{
		...
		if (y2 >= 0 && x2 >= 0 && x1 < Width && y1 < Height)
		{
			...
			{
				Buffer[i, y1].Char = 'ß';
				Buffer[i, y1].Attributes = Color;
				Buffer[i, y1].HFlip = false;
				Buffer[i, y1].VFlip = false;
				Buffer[i, y2].Char = 'Ü';
				Buffer[i, y2].Attributes = Color;
				Buffer[i, y2].HFlip = false;
				Buffer[i, y2].VFlip = false;
			}
			for (int j = y1 + 1; j < y2; j++)
			{
				Buffer[x1, j].Char = 'Ý';
				Buffer[x1, j].Attributes = Color;
				Buffer[x1, j].HFlip = false;
				Buffer[x1, j].VFlip = false;
				Buffer[x2, j].Char = 'Þ';
				Buffer[x2, j].Attributes = Color;
				Buffer[x2, j].HFlip = false;
				Buffer[x2, j].VFlip = false;
			}
			Buffer[x1, y1].Char = 'Û';
			Buffer[x1, y1].Attributes = Color;
			Buffer[x1, y1].HFlip = false;
			Buffer[x1, y1].VFlip = false;
			Buffer[x2, y1].Char = 'Û';
			Buffer[x2, y1].Attributes = Color;
			Buffer[x2, y1].HFlip = false;
			Buffer[x2, y1].VFlip = false;
			Buffer[x1, y2].Char = 'Û';
			Buffer[x1, y2].Attributes = Color;
			Buffer[x1, y2].HFlip = false;
			Buffer[x1, y2].VFlip = false;
			Buffer[x2, y2].Char = 'Û';
			Buffer[x2, y2].Attributes = Color;
			Buffer[x2, y2].HFlip = false;
			Buffer[x2, y2].VFlip = false;
		}
	}
