...
public class TestInputField : Selectable, IUpdateSelectedHandler, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler, ISubmitHandler, ICanvasElement, ILayoutElement
{
	...
	private void Insert(char c)
	{
		if (!m_ReadOnly)
		{
			...
			if (characterLimit <= 0 || this.text.Length < characterLimit)
			{
				...
				int num2 = (caretSelectPositionInternal = (caretPositionInternal += text.Length));
				SendOnValueChanged();
			}
		}
	}
	...
	private void UpdateGeometry()
	{
		if (shouldHideMobileInput)
		{
			if (m_CachedInputRenderer == null && m_TextComponent != null)
			{
				...
				gameObject.transform.SetParent(m_TextComponent.transform.parent, worldPositionStays: false);
