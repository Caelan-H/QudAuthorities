...
public class Tween<T> : CustomYieldInstruction
{
	...
	private static float easeOutElastic(float start, float distance, float elapsedTime, float duration)
	{
		...
		float num2 = num / 4f;
		return distance * Mathf.Pow(2f, -10f * elapsedTime) * Mathf.Sin((elapsedTime * duration - num2) * ((float)Math.PI * 2f) / num) + distance + start;
	}
	...
	private static float easeInElastic(float start, float distance, float elapsedTime, float duration)
	{
		...
		float num = duration * 0.3f;
		float num2 = num / 4f;
		return 0f - distance * Mathf.Pow(2f, 10f * (elapsedTime -= 1f)) * Mathf.Sin((elapsedTime * duration - num2) * ((float)Math.PI * 2f) / num) + start;
	}
	...
	private static float easeInOutElastic(float start, float distance, float elapsedTime, float duration)
	{
		...
		float num = duration * 0.450000018f;
		float num2 = num / 4f;
		if (elapsedTime < 1f)
		{
			return -0.5f * (distance * Mathf.Pow(2f, 10f * (elapsedTime -= 1f)) * Mathf.Sin((elapsedTime * duration - num2) * ((float)Math.PI * 2f) / num)) + start;
		}
		return distance * Mathf.Pow(2f, -10f * (elapsedTime -= 1f)) * Mathf.Sin((elapsedTime * duration - num2) * ((float)Math.PI * 2f) / num) * 0.5f + distance + start;
	}
	...
	private static float easeOutBounce(float start, float distance, float elapsedTime, float duration)
	{
		...
		if ((elapsedTime /= duration) < 0.363636374f)
		{
			return distance * (7.5625f * elapsedTime * elapsedTime) + start;
		}
		if (elapsedTime < 0.727272749f)
		{
			return distance * (7.5625f * (elapsedTime -= 0.545454562f) * elapsedTime + 0.75f) + start;
		}
		if (elapsedTime < 0.909090936f)
		{
