...
public class ExtradimensionalHunterSummoner : IPart
{
	...
	public void Summon(int Number = 1)
	{
		...
		for (int i = 0; i < Number; i++)
		{
			...
			{
				IComponent<GameObject>.XDidYToZ(gameObject, "emerge", "from", randomElement, null, null, null, null, null, UseFullNames: false, IndefiniteSubject: false, IndefiniteObject: true, IndefiniteObjectForOthers: false, PossessiveObject: false, null, null, null, DescribeSubjectDirection: false, DescribeSubjectDirectionLate: true);
				continue;
			}
		...
		if (list.Count > 0)
		{
			DidX("open", "wide, and the nacre inside " + ParentObject.its + " shell glimmers with unseen light", null, null, null, null, UseFullNames: false, IndefiniteSubject: false, null, null, DescribeSubjectDirection: false, DescribeSubjectDirectionLate: false, AlwaysVisible: false, FromDialog: false, UsePopup: true);
	...
	{
		if (!Active && E.Actor != null && E.Actor.IsPlayerControlled())
		{
			Activate();
		}
	...
	public override bool FireEvent(Event E)
	{
		if (!Active && E.ID == "BeforeDeathRemoval" && ActiveOnDeathChance.in100())
		{
			...
			{
				ParentObject.GetAngryAt(gameObjectParameter, -100);
			}
			Activate();
		}
