...
[Serializable]
public class AIWanderingJuggernaut : IPart
{
	public int Angle;
	...
	{
		if (ParentObject.IsPlayer() || ParentObject.pBrain == null || !ParentObject.FireEvent("CanAIDoIndependentBehavior") || ParentObject.IsFleeing())
		{
			return;
		}
		...
		if (target != null)
		{
			...
			if (cell == cell3)
			{
				ParentObject.pBrain.PushGoal(new FleeLocation(cell, 1));
			}
			else if (cell2 != cell3 && cell.IsAdjacentTo(cell3))
			{
				...
				if (directionFromCell != null)
				{
					...
					ParentObject.pBrain.PushGoal(new Step(directionFromCell, careful: false, overridesCombat: true, wandering: false, juggernaut: true, target));
				}
			}
		}
		else if (cell2 == null)
		{
			...
			{
				ParentObject.pBrain.PushGoal(new Step(list[num2], careful: false, overridesCombat: false, wandering: true, juggernaut: true, null, allowUnbuilt: true));
			}
		}
	}
}
