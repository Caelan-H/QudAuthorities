...
public class Interesting : IPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != GetDebugInternalsEvent.ID && ID != GetPointsOfInterestEvent.ID)
		{
	...
	public override bool HandleEvent(GetPointsOfInterestEvent E)
	{
		if (RequirementsMet(E.Actor))
		{
			...
			if (flag)
			{
				...
				E.Add(@object, DisplayName ?? ParentObject.BaseDisplayName, text, text2, Preposition, location, null, Radius, renderable, Order);
			}
		}
		return base.HandleEvent(E);
	}
	...
	public bool RequirementsMet(GameObject who)
	{
		...
		if (!string.IsNullOrEmpty(IfPartOperational) && (!(ParentObject.GetPart(IfPartOperational) is IActivePart activePart) || activePart.IsDisabled(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L)))
		{
			return false;
		}
		return true;
