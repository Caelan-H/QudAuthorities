...
public class CyberneticsElectromagneticSensor : IPart
{
	...
	public override void TurnTick(long TurnNumber)
	{
		...
		foreach (GameObject item in heardObjects)
		{
			...
			{
				item.RemoveEffect("SenseRobotEffect", IsOurs);
			}
		}
		int i = 0;
		for (int count = HeardObjects.Count; i < count; i++)
		{
			...
			if (!gameObject.HasEffect("SenseRobotEffect", IsOurs) && WillSense(gameObject))
			{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != ExtraHostilePerceptionEvent.ID && ID != GetShortDescriptionEvent.ID)
		{
		...
		if (implantee != null && E.Actor == implantee && implantee.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				for (int count = list.Count; i < count; i++)
				{
					...
					if (gameObject.GetEffect("SenseRobotEffect", IsOurs) is SenseRobotEffect senseRobotEffect && senseRobotEffect.Listener == implantee && senseRobotEffect.Identified && implantee.IsRelevantHostile(gameObject))
					{
	...
	public override bool HandleEvent(UnimplantedEvent E)
	{
		...
		{
			heardObject.RemoveEffect("SenseRobotEffect", IsOurs);
		}
		HeardObjects.Clear();
		return base.HandleEvent(E);
	}
