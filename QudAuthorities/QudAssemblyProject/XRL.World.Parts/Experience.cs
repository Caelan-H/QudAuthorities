	...
	public override bool HandleEvent(AwardXPEvent E)
	{
		try
		{
			...
			if (AwardingXPEvent.Check(E))
			{
				...
				if (E.PassedDownFrom == null && E.PassedUpFrom != ParentObject && ParentObject.pBrain != null)
				{
					GameObject partyLeader = ParentObject.PartyLeader;
					if (partyLeader != null)
					{
						partyLeader.AwardXP(E.Amount, E.Tier, 0, int.MaxValue, E.Kill, E.InfluencedBy, E.PassedUpFrom ?? ParentObject, null, E.Deed);
					}
					else if (E.PassedUpFrom == null && ParentObject.IsPlayer())
					{
						...
						if (cell != null && cell.ParentZone != null)
						{
							foreach (GameObject item in cell.ParentZone.FastSquareSearch(cell.X, cell.Y, 10, "Brain"))
							{
								...
								{
									item.AwardXP(E.Amount, E.Tier, 0, int.MaxValue, E.Kill, E.InfluencedBy, null, ParentObject, E.Deed);
								}
							}
						}
					}
