...
public class NightVision : IPoweredPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != BeforeApplyDamageEvent.ID && ID != BeforeRenderEvent.ID && ID != EarlyBeforeBeginTakeActionEvent.ID && ID != EquippedEvent.ID)
		{
	...
	{
		if (IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L) && (E.Damage.HasAttribute("Light") || E.Damage.HasAttribute("Laser")))
		{
