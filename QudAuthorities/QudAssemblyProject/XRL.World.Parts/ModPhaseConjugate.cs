...
public class ModPhaseConjugate : IModification
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != BeforeDetonateEvent.ID && ID != GenericQueryEvent.ID && ID != GetDisplayNameEvent.ID)
		{
	...
	public override bool HandleEvent(BeforeDetonateEvent E)
	{
		...
		{
			if (!(ParentObject.GetEffect("Phased") is Phased e))
			{
				ParentObject.ForceApplyEffect(new Phased(1));
			}
			else
			{
				ParentObject.RemoveEffect(e);
