...
public class Toolbox : IPoweredPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != GetDebugInternalsEvent.ID && (ID != GetShortDescriptionEvent.ID || string.IsNullOrEmpty(BehaviorDescription)))
		{
	...
	private bool HandleBonus(GetTinkeringBonusEvent E, int PoweredBonus, int UnpoweredBonus)
	{
		...
		{
			if ((!TrackAsToolbox || ((PoweredBonus >= 0) ? (E.ToolboxBonus < PoweredBonus) : (E.ToolboxBonus > PoweredBonus))) && IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: true, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L) && IsObjectActivePartSubject(E.Actor))
			{
				ActivePartStatus activePartStatus = GetActivePartStatus(UseCharge: true, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, ParentObject.Count, null, UseChargeIfUnpowered: false, 0L);
		...
		else if (IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: true, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L) && IsObjectActivePartSubject(E.Actor))
		{
			ApplyBonus(E, UnpoweredBonus);
		}
		return true;
