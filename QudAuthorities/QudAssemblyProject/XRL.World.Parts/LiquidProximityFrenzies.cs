...
public class LiquidProximityFrenzies : IPart
{
	...
	public GameObject GetProximateLiquidPool()
	{
		...
		for (int count = cell.Objects.Count; i < count; i++)
		{
			...
			if (gameObject.GetEffect("LiquidCovered") is LiquidCovered liquidCovered)
			{
				LiquidVolume liquid = liquidCovered.Liquid;
		...
		for (int j = 1; j <= Range; j++)
		{
			...
			for (int count2 = list3.Count; k < count2; k++)
			{
				...
				for (int count3 = cell2.Objects.Count; l < count3; l++)
				{
					...
					if (gameObject2.GetEffect("LiquidCovered") is LiquidCovered liquidCovered2)
					{
						LiquidVolume liquid2 = liquidCovered2.Liquid;
	...
	public bool TriggerEffect(GameObject obj = null)
	{
		...
		if (!string.IsNullOrEmpty(Message) && Visible())
		{
			string text = Message;
			...
			text = GameText.VariableReplace(text, ParentObject, null, ExplicitSubjectPlural: false, obj);
