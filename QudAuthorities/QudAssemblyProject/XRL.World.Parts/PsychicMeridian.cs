...
public class PsychicMeridian : IActivePart
{
	...
	{
		CurrentCooldown = Math.Max(0, CurrentCooldown - 10);
	}

	public override void HundredTurnTick(long TurnNumber)
	{
		CurrentCooldown = Math.Max(0, CurrentCooldown - 100);
	}

	public override bool WantEvent(int ID, int cascade)
	{
		if (!base.WantEvent(ID, cascade) && ID != AfterMentalDefendEvent.ID && ID != GetItemElementsEvent.ID && ID != EquippedEvent.ID)
		{
	...
	{
		if ((!Guarded || E.Penetrations <= 0) && CurrentCooldown <= 0 && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L))
		{
	...
	public void AfflictNosebleed(GameObject Attacker)
	{
		...
		{
			IComponent<GameObject>.XDidY(gameObject, "impale", gameObject.itself + " on " + activePartFirstSubject.poss("psychic barbs"), null, null, null, gameObject);
		}
	...
	public override bool FireEvent(Event E)
	{
		if ((E.ID == "DefenderAfterAttack" || E.ID == "DefenderProjectileHit") && CurrentCooldown <= 0)
		{
			...
			if (IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L) && E.GetParameter("Damage") is Damage damage && damage.HasAttribute("Mental"))
			{
