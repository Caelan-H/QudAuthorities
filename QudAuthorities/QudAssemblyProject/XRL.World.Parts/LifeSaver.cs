...
public class LifeSaver : IPoweredPart
{
	...
	public bool CheckLifeSaverActivation(GameObject who, Damage dmg)
	{
		...
		else if (!string.IsNullOrEmpty(MaxHitpointsThreshold) && dmg.Amount >= who.baseHitpoints * MaxHitpointsThreshold.RollCached() / 100)
		{
			chance = ChanceVsMaxHitpointsThreshold;
			text = MaxHitpointsThresholdMessage;
		}
		else if (!string.IsNullOrEmpty(CurrentHitpointsThreshold) && dmg.Amount >= hitpoints * CurrentHitpointsThreshold.RollCached() / 100)
		{
		...
		if (!string.IsNullOrEmpty(text))
		{
			IComponent<GameObject>.EmitMessage(who, GameText.VariableReplace(text, who, null, ExplicitSubjectPlural: false, ParentObject), FromDialog: true);
		}
		Used++;
		if (DestroyWhenUsedUp && Used >= MaxUses)
		{
			if (!string.IsNullOrEmpty(DestroyWhenUsedUpMessage))
			{
				IComponent<GameObject>.EmitMessage(who, GameText.VariableReplace(DestroyWhenUsedUpMessage, who, null, ExplicitSubjectPlural: false, ParentObject), FromDialog: true);
			}
