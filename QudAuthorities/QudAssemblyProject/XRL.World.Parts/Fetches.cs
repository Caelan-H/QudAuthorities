...
public class Fetches : IPart
{
	...
	public override bool HandleEvent(AIBoredEvent E)
	{
		if (ParentObject.CurrentCell != null && 7.in100())
		{
			...
			GameObject randomElement = ParentObject.CurrentZone.GetObjectsNoAlloc(ShouldFetch).GetRandomElement();
			if (randomElement != null)
			{
				IComponent<GameObject>.AddPlayerMessage(GameText.VariableReplace(FetchMessage, ParentObject) + randomElement.a + randomElement.ShortDisplayName + "!");
				ParentObject.pBrain.PushGoal(new GoFetch(randomElement.GetCurrentCell()));
			}
