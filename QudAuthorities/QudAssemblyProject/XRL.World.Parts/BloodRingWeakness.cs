...
public class BloodRingWeakness : IPart
{
	public override bool WantEvent(int ID, int cascade)
	{
		...
		{
			return ID == BeforeBeginTakeActionEvent.ID;
		}
		return true;
	}
	...
	public override bool HandleEvent(BeforeBeginTakeActionEvent E)
	{
		...
		if (cell != null && ParentObject.pBrain != null && !ParentObject.HasEffect("Terrified"))
		{
			foreach (GameObject item in cell.ParentZone.FastSquareVisibility(cell.X, cell.Y, 5, "Body", ParentObject))
			{
				...
				{
					ParentObject.ApplyEffect(new Terrified(2, item, Panicked: false, Psionic: false, Silent: true));
					break;
				}
			}
		}
