		...
		if (E.ID == "EndTurnEngulfing")
		{
			...
			if (TurnsLeft == 0)
			{
				...
				if (ParentObject.CurrentCell != null && part != null && part.Engulfed != null && part.Engulfed.CanBeInvoluntarilyMoved())
				{
					...
					if (cellFromDirection != null && cellFromDirection.IsPassable(ParentObject) && cellFromDirection.GetCombatTarget(ParentObject, IgnoreFlight: false, IgnoreAttackable: false, IgnorePhase: false, 0, null, null, AllowInanimate: false) == null)
					{
						...
						{
							DidXToY("melt", "through the floor and" + ParentObject.GetVerb("descend") + " with", part.Engulfed, null, null, null, null, part.Engulfed);
						}
