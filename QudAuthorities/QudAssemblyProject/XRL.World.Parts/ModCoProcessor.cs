...
public class ModCoProcessor : IModification
{
	...
	public override bool ModificationApplicable(GameObject Object)
	{
		...
		if (!string.IsNullOrEmpty(usesSlots) && !usesSlots.CachedCommaExpansion().Contains("Head"))
		{
			return false;
		}
		return true;
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != BootSequenceDoneEvent.ID && ID != BootSequenceInitializedEvent.ID && ID != EffectAppliedEvent.ID && ID != EffectRemovedEvent.ID && ID != EquippedEvent.ID && ID != GetAvailableComputePowerEvent.ID && ID != GetDisplayNameEvent.ID && ID != GetItemElementsEvent.ID && ID != GetShortDescriptionEvent.ID && ID != ModificationAppliedEvent.ID)
		{
	...
	{
		return "Co-Processor: When powered, this item grants " + ((Tier == 0) ? "bonus" : ("+" + GetAttributeBonus(Tier, 100, 0.25f))) + " Intelligence and provides " + ((Tier == 0) ? "" : (GetComputePower(Tier, 100, 2.5f) + " units of ")) + "compute power to the local lattice.";
	}

	public static string GetDescription(int Tier, GameObject obj)
	{
		int powerLoadLevel = obj.GetPowerLoadLevel();
		return "Co-Processor: When powered, this item grants " + ((Tier == 0) ? "bonus" : ("+" + GetAttributeBonus(Tier, powerLoadLevel))) + " Intelligence and provides " + ((Tier == 0) ? "" : (GetComputePower(Tier, powerLoadLevel) + " units of ")) + "compute power to the local lattice.";
	}
