...
public class WaterRitualRecord : IPart
{
	...
	{
		base.Initialize();
		Faction faction = Factions.get(ParentObject.GetPrimaryFaction() ?? "Beasts");
		this.faction = faction.Name;
		totalFactionAvailable = 100;
	...
	public override bool HandleEvent(BeginConversationEvent E)
	{
		...
		{
			Popup.Show("You bothered " + ParentObject.the + ParentObject.ShortDisplayName + " again.");
			The.Game.PlayerReputation.modify(faction, -hermitChatPenalty);
			hermitChatPenalty = 0;
		}
