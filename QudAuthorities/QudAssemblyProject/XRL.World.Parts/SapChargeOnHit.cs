...
public class SapChargeOnHit : IActivePart
{
	...
	{
		if (O == null || D == null || O.IsGraveyard() || D.IsGraveyard())
		{
			return;
		}
		List<Point> list = Zone.Line(O.X, O.Y, D.X, D.Y);
		for (int i = 0; i < list.Count; i++)
		{
			...
			if (firstObject != null)
			{
				...
				{
					firstObject.ParticleBlip("&W" + (char)Stat.RandomCosmetic(191, 198), 30);
				}
				else
				{
					firstObject.ParticleBlip("&Y" + (char)Stat.RandomCosmetic(191, 198), 30);
				}
			}
		}
	}
