...
public class Banner : IActivePart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != DroppedEvent.ID && ID != EndTurnEvent.ID && ID != EquippedEvent.ID && ID != GetDisplayNameEvent.ID && ID != GetInventoryActionsEvent.ID && ID != GetShortDescriptionEvent.ID && ID != InventoryActionEvent.ID && ID != TakenEvent.ID)
		{
	...
	public override bool HandleEvent(InventoryActionEvent E)
	{
		...
		{
			IComponent<GameObject>.XDidYToZ(E.Actor, "raise", ParentObject, null, null, null, E.Actor);
			Raised = true;
		}
		return base.HandleEvent(E);
	}
	...
	public void ApplyEffectTo(GameObject target)
	{
		...
		{
			target.GetEffect(Effect).Duration = 2;
			return;
		}
		Effect effect = Activator.CreateInstance(effectType) as Effect;
		effect.Duration = 2;
