...
public class EnergyCellRack : IPoweredPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && (ID != CanAcceptObjectEvent.ID || !PreventOther) && ID != EndTurnEvent.ID && ID != GetContentsEvent.ID && ID != GetDisplayNameEvent.ID && ID != GetExtrinsicValueEvent.ID && ID != GetExtrinsicWeightEvent.ID && ID != GetInventoryActionsEvent.ID && ID != InventoryActionEvent.ID && ID != QueryChargeEvent.ID && ID != TestChargeEvent.ID)
		{
	...
	{
		if (IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, E.GridMask))
		{
