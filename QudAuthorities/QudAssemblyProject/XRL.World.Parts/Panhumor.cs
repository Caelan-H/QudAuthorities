...
public class Panhumor : IPart
{
	...
	private void CheckPodInit(ref GameObject obj)
	{
		if (obj == null || obj.IsInvalid())
		{
			...
			ParentObject.TakeObject(obj, Silent: true, 0);
		}
	}
	...
	private void SetupPod(GameObject obj)
	{
		...
		ElementalDamage elementalDamage = obj.GetPart("ElementalDamage") as ElementalDamage;
		if (Form == "Fire")
		{
			obj.pRender.DisplayName = "{{R|flaming pseudopod}}";
			if (elementalDamage != null)
			{
				elementalDamage.Attributes = "Fire";
			}
			obj.AddPart(new TemperatureOnHit(Level + "d20+" + Level * 50, 400));
		}
		else if (Form == "Ice")
		{
			obj.pRender.DisplayName = "{{C|hoary pseudopod}}";
			if (elementalDamage != null)
			{
				elementalDamage.Attributes = "Cold";
			}
			obj.AddPart(new TemperatureOnHit("-" + Level + "d20-" + Level * 20, 400));
		}
		else if (Form == "Acid")
		{
			obj.pRender.DisplayName = "{{G|acidic pseudopod}}";
			if (elementalDamage != null)
			{
				elementalDamage.Attributes = "Acid";
			}
		}
		else if (Form == "Shocking")
		{
			obj.pRender.DisplayName = "{{W|sparking pseudopod}}";
			if (elementalDamage != null)
			{
				elementalDamage.Attributes = "Shock";
			}
	...
	public override bool FireEvent(Event E)
	{
		if (!(E.ID == "BeginTakeAction"))
		{
			if (E.ID == "EnteredCell")
			{
				ParentObject.CurrentCell.AddObject("AcidPool");
			}
			...
			{
				if (ParentObject.pPhysics.CurrentCell == null || ParentObject.pPhysics.CurrentCell.ParentZone == null || !ParentObject.pPhysics.CurrentCell.ParentZone.IsActive())
				{
					return true;
				}
				bool flag = true;
				...
				{
					ParentObject.Discharge(ParentObject.CurrentCell.GetRandomLocalAdjacentCell(), 20, Level + "d20", ParentObject);
				}
				if (Form == "Fire" || flag)
				{
					DidX("explode", null, "!", null, null, IComponent<GameObject>.ThePlayer);
					if (ParentObject.GetPart("Physics") is Physics physics && physics.CurrentCell != null)
					{
						...
						{
							item.TemperatureChange(num, gameObjectParameter, Radiant: false, MinAmbient: false, MaxAmbient: false, phase);
							if (num > 0)
							{
				...
				{
					DidX("explode", null, "!", null, null, IComponent<GameObject>.ThePlayer);
					if (ParentObject.GetPart("Physics") is Physics physics2 && physics2.CurrentCell != null)
					{
						...
						{
							item2.TemperatureChange(num2, gameObjectParameter2, Radiant: false, MinAmbient: false, MaxAmbient: false, phase2);
							if (num2 > 0)
							{
				...
				if ((Form == "Acid" || flag) && ParentObject.pPhysics.CurrentCell != null)
				{
					List<Cell> adjacentCells3 = ParentObject.pPhysics.CurrentCell.GetAdjacentCells();
					adjacentCells3.Add(ParentObject.pPhysics.CurrentCell);
					foreach (Cell item3 in adjacentCells3)
					{
						if (!item3.IsOccluding() && Stat.Random(0, 100) <= 75)
						{
							item3.AddObject(GameObjectFactory.Factory.CreateObject("AcidPool"));
						}
					}
				}
			}
