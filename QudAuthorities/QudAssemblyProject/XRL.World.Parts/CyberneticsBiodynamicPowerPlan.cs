...
public class CyberneticsBiodynamicPowerPlant : IPoweredPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != BeginTakeActionEvent.ID && ID != EffectAppliedEvent.ID && ID != GetDebugInternalsEvent.ID && ID != ImplantedEvent.ID)
		{
	...
	public override bool FireEvent(Event E)
	{
		...
		if (E.ID == "QueryCharge")
		{
			if ((ConsiderLive || !E.HasFlag("LiveOnly")) && E.HasFlag("IncludeTransient") && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L))
			{
		...
		else if (E.ID == "OwnerGetShortDescription")
		{
			...
			if (gameObjectParameter.GetPart("IntegratedPowerSystems") is IntegratedPowerSystems integratedPowerSystems && integratedPowerSystems.RequiresEvent == "HasPowerConnectors" && !gameObjectParameter.HasPart("ModJacked"))
			{
		...
		else if (E.ID == "QueryChargeProduction" && (ConsiderLive || !E.HasFlag("LiveOnly")) && E.HasFlag("IncludeTransient") && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L))
		{
