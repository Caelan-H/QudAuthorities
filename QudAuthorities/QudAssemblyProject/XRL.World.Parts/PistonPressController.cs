	...
	public override bool FireEvent(Event E)
	{
		if (E.ID == "EndTurn")
		{
			if (elements == null && Directions != null)
			{
				...
				{
					Cell cellFromDirection = ParentObject.pPhysics.currentCell.GetCellFromDirection(Directions[i].ToString());
			...
			if (elements != null)
			{
				...
				foreach (string key in elements.Keys)
				{
					...
					if (pistonPressElement == null || pistonPressElement.ParentObject == null || pistonPressElement.ParentObject.pPhysics == null || pistonPressElement.ParentObject.pPhysics.CurrentCell == null || pistonPressElement.ParentObject.pPhysics.CurrentCell != ParentObject.pPhysics.currentCell.GetCellFromDirection(key))
					{
						if (list == null)
						{
			...
			if (elements != null && elements.Count != 0)
			{
				...
				else if (Counter >= Frequency)
				{
					...
					ParentObject.pRender.RenderString = centerGlyph[Directions];
					ParentObject.pRender.Tile = centerTile[Directions];
					if (centerElement == null)
					{
					...
					if (centerElement != null)
					{
						centerElement.pRender.RenderString = centerGlyph[Directions];
						centerElement.pRender.Tile = centerTile[Directions];
					}
					...
					if (elements.Count == 1)
					{
						if (centerElement != null)
						{
							centerElement.pRender.RenderString = centerGlyphOne[elements.Keys.FirstOrDefault()];
							centerElement.pRender.Tile = centerTileOne[elements.Keys.FirstOrDefault()];
						}
						ParentObject.pRender.RenderString = centerGlyphOne[elements.Keys.FirstOrDefault()];
						ParentObject.pRender.Tile = centerTileOne[elements.Keys.FirstOrDefault()];
					...
					else
					{
						...
						{
							if (item2 != ParentObject && item2.pPhysics != null && item2.pRender != null && item2.pRender.RenderLayer > 5)
							{
								item2.TakeDamage(Stat.Random(80, 120), "from being crushed by a machine press.", "Bludgeoning", null, null, null, ParentObject, null, null, Accidental: false, Environmental: false, Indirect: false, ShowUninvolved: false, ShowForInanimate: false, SilentIfNoDamage: false, phase);
							}
						}
					}
				}
