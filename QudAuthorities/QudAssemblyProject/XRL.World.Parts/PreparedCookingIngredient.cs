...
public class PreparedCookingIngredient : IPart
{
	...
	{
		Object.RegisterPartEvent(this, "SelfRenderStackerDisplay");
	...
	public override bool FireEvent(Event E)
	{
		...
		{
			int num = 0;
			int number = ParentObject.GetPart<Stacker>().Number;
			num = charges;
			if (number > 1)
			{
				if (num == 1)
	...
	public static List<string> GetRandomTypeList()
	{
		if (RandomTypeList == null)
		{
			...
			{
				if (!value.InheritsFrom("IngredientMapping"))
				{
					continue;
				}
				string tag = value.GetTag("RandomWeight");
				...
				try
				{
					...
					for (int i = 0; i < num; i++)
					{
						RandomTypeList.Add(value.Name.Split('_')[1]);
					}
				}
				catch
				{
