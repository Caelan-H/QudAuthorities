...
public class QuestGiverImposter : IPartWithPrefabImposter
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != AfterConversationEvent.ID && ID != BeginTakeActionEvent.ID && ID != EnteredCellEvent.ID)
		{
	...
	public void UpdateIcon()
	{
		...
		foreach (string questID in givesQuestList)
		{
			if (!IComponent<GameObject>.TheGame.HasQuest(questID))
			{
