...
public abstract class IPowerTransmission : IPoweredPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && (ID != AllowLiquidCollectionEvent.ID || string.IsNullOrEmpty(DependsOnLiquid)) && ID != AnimateEvent.ID && (ID != ChargeAvailableEvent.ID || !IsProducer) && ID != EnteredCellEvent.ID && ID != EffectAppliedEvent.ID && ID != EffectRemovedEvent.ID && (ID != FinishChargeAvailableEvent.ID || !IsProducer) && ID != GetDebugInternalsEvent.ID && (ID != GetPreferredLiquidEvent.ID || string.IsNullOrEmpty(DependsOnLiquid)) && ID != GetShortDescriptionEvent.ID && ID != LeavingCellEvent.ID && (ID != LiquidMixedEvent.ID || string.IsNullOrEmpty(DependsOnLiquid)) && ID != OnDestroyObjectEvent.ID && (ID != PowerSwitchFlippedEvent.ID || !IsPowerSwitchSensitive) && (ID != RepaintedEvent.ID || !TileEffects || string.IsNullOrEmpty(TileBaseFromTag)) && (ID != StatChangeEvent.ID || !DamageDegrades) && (ID != QueryChargeEvent.ID || !IsConsumer) && (ID != TestChargeEvent.ID || !IsConsumer) && (ID != UseChargeEvent.ID || !IsConsumer))
		{
			if (ID == WantsLiquidCollectionEvent.ID)
			{
				return !string.IsNullOrEmpty(DependsOnLiquid);
			}
			return false;
		}
		return true;
	...
	{
		if ((E.GridMask & GridBit) == 0L && IsConsumer && IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, E.GridMask | GridBit))
		{
	...
	{
		if (E.Pass == 1 && (E.GridMask & GridBit) == 0L && IsConsumer && IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, E.GridMask | GridBit))
		{
	...
	{
		if (!string.IsNullOrEmpty(DependsOnLiquid))
		{
			ResetGrid();
		}
		return base.HandleEvent(E);
	...
	{
		if (TileEffects && !string.IsNullOrEmpty(TileBaseFromTag))
		{
			string tag = ParentObject.GetTag(TileBaseFromTag);
			if (tag != null)
			{
				TileSplitter[0] = tag;
				PaintedTileParts = ParentObject.pRender.Tile.Split(TileSplitter, 2, StringSplitOptions.None);
			}
			else
			{
	...
	{
		if (E.Liquid == null && !string.IsNullOrEmpty(DependsOnLiquid))
		{
	...
	public override bool HandleEvent(GetShortDescriptionEvent E)
	{
		...
		if (Readout || (!string.IsNullOrEmpty(DescribeStatusForProperty) && IComponent<GameObject>.ThePlayer != null && IComponent<GameObject>.ThePlayer.GetIntProperty(DescribeStatusForProperty) > 0))
		{
	...
	{
		if ((GridMask & GridBit) == 0L && (IgnoreReady || IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, GridMask | GridBit)))
		{
			CheckGrid();
			if (Producers != null)
			{
	...
	{
		int num = 0;
		if ((GridMask & GridBit) == 0L && IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, GridMask | GridBit))
		{
			CheckGrid();
			if (Producers != null)
			{
	...
	public int GetTotalProduction()
	{
		...
		if (Producers != null)
		{
			...
			{
				num += producer.QueryChargeProduction();
			}
			return num;
		}
		return num;
	...
	{
		if (!DamageDegrades && string.IsNullOrEmpty(DependsOnLiquid))
		{
			return 1.0;
		}
		double num = 1.0;
	...
	public void SyncTile()
	{
		...
		if (flag2)
		{
			if (!string.IsNullOrEmpty(TileAppendWhenBrokenAndPowered) || !string.IsNullOrEmpty(TileAppendWhenUnbrokenAndPowered))
			{
				if (IsBroken())
				{
					if (!string.IsNullOrEmpty(TileAppendWhenBrokenAndPowered))
					{
						TileBuilder.Append(TileAppendWhenBrokenAndPowered);
						flag = true;
					}
				}
				else if (!string.IsNullOrEmpty(TileAppendWhenUnbrokenAndPowered))
				{
					TileBuilder.Append(TileAppendWhenUnbrokenAndPowered);
					flag = true;
				}
			}
			if (!flag && !string.IsNullOrEmpty(TileAppendWhenPowered))
			{
		...
		{
			if (!string.IsNullOrEmpty(TileAppendWhenBrokenAndUnpowered) || !string.IsNullOrEmpty(TileAppendWhenUnbrokenAndUnpowered))
			{
				if (IsBroken())
				{
					if (!string.IsNullOrEmpty(TileAppendWhenBrokenAndUnpowered))
					{
						TileBuilder.Append(TileAppendWhenBrokenAndUnpowered);
						flag = true;
					}
				}
				else if (!string.IsNullOrEmpty(TileAppendWhenUnbrokenAndUnpowered))
				{
					TileBuilder.Append(TileAppendWhenUnbrokenAndUnpowered);
					flag = true;
				}
			}
			if (!flag && !string.IsNullOrEmpty(TileAppendWhenUnpowered))
			{
				TileBuilder.Append(TileAppendWhenUnpowered);
			}
		}
		if (!flag && (!string.IsNullOrEmpty(TileAppendWhenBroken) || !string.IsNullOrEmpty(TileAppendWhenUnbroken)))
		{
			if (IsBroken())
			{
				if (!string.IsNullOrEmpty(TileAppendWhenBroken))
				{
					TileBuilder.Append(TileAppendWhenBroken);
				}
			}
			else if (!string.IsNullOrEmpty(TileAppendWhenUnbroken))
			{
	...
	{
		return obj.GetPartDescendedFrom<IPowerTransmission>(TypeMatches);
	}
		...
		while (VisitNext.Count > 0)
		{
			...
			for (int count = VisitNow.Count; i < count; i++)
			{
				...
				for (int count2 = cell.Objects.Count; j < count2; j++)
				{
					...
					IPowerTransmission correspondingPart = GetCorrespondingPart(gameObject);
					if (correspondingPart == null || !correspondingPart.IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, GridBit))
					{
						continue;
					}
	...
	public void ResetGrid(bool Local = false)
	{
		if (ParentObject.GetLongProperty("LastReset") == The.Game.Turns)
		{
			return;
		}
	...
	private void ResetGrid(Cell C)
	{
		...
		{
			GameObject gameObject = C.Objects[i];
			if (gameObject.GetLongProperty("LastReset") != The.Game.Turns && gameObject != ParentObject)
			{
				IPowerTransmission correspondingPart = GetCorrespondingPart(gameObject);
				if (correspondingPart != null && correspondingPart.GridBit != 0L)
				{
					correspondingPart.ResetGrid();
				}
			}
		}
	}
	...
	public void Spark()
	{
		...
		if (randomLocalAdjacentCell != null)
		{
			...
			{
				text = text + "+" + Math.Min(adjacentCharge / 200, 10);
			}
			ParentObject.Discharge(randomLocalAdjacentCell, voltage, text, ParentObject);
		}
	...
	{
		C.ForeachObject((Action<GameObject>)MingleScan);
	}
	...
	public void MingleLiquids()
	{
		...
		{
			ParentObject.CurrentCell.ForeachCardinalAdjacentCell((Action<Cell>)MingleScan);
		}
	}
	...
	private void Process(IChargeProductionEvent E)
	{
		...
		if ((gridMask & GridBit) != 0L || !IsReady(UseCharge: false, IgnoreCharge: true, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, E.GridMask | GridBit))
		{
			return;
		}
		CheckGrid();
