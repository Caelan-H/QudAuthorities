...
[Serializable]
public class AIUrnDuster : IPart
{
	...
	public bool CheckDustUrn()
	{
		...
		ParentObject.pBrain.PushGoal(new DustAnUrnGoal(globalLocation));
	...
	private static List<GlobalLocation> FindGlobalUrns()
	{
		...
		{
			if (!XRLCore.Core.Game.ZoneManager.IsZoneLive(item))
			{
				continue;
			}
			Zone zone = XRLCore.Core.Game.ZoneManager.GetZone(item);
