...
public class StickyOnHit : IPart
{
	...
	public void Entangle(GameObject Target = null)
	{
		if (Target == null)
		{
			...
			if (cell != null)
			{
				Target = cell.GetCombatTarget(ParentObject, IgnoreFlight: true, IgnoreAttackable: false, IgnorePhase: false, 0, null, null, AllowInanimate: false);
			}
		}
		if (Target != null && Target.ApplyEffect(new Stuck(Duration.RollCached(), SaveTarget.RollCached(), SaveVs, ParentObject)))
		{
			IComponent<GameObject>.XDidYToZ(Target, "get", "entangled in", ParentObject, null, "!", null, null, Target);
		}
	}
}
