...
public class ThermalGrenade : IGrenade
{
	...
	protected override bool DoDetonate(Cell C, GameObject Actor = null, GameObject ApparentTarget = null, bool Indirect = false)
	{
		...
		PlayWorldSound(GetPropertyOrTag("DetonatedSound", "grenade_heat"), 1f, 0f, combat: true);
		...
		{
			item.TemperatureChange(TemperatureDelta, Actor, Radiant: false, MinAmbient: false, MaxAmbient: false, phase);
			if (!item.IsVisible())
			{
				continue;
			}
