...
public class MerchantRevealer : IPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != AddedToInventoryEvent.ID && ID != BeforeObjectCreatedEvent.ID && ID != EnteredCellEvent.ID && ID != EquippedEvent.ID && ID != GetInventoryActionsEvent.ID && ID != HasBeenReadEvent.ID)
		{
	...
	{
		E.AddAction("Read", "read", "Read", null, 'r', FireOnActor: false, (merchantLoc == null || merchantLoc.revealed) ? 1 : 100);
		return base.HandleEvent(E);
	}
