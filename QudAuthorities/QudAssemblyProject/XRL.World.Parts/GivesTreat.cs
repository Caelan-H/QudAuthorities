...
public class GivesTreat : IPart
{
	...
	{
		if (E.ID == "BeforeTakeAction" && ParentObject.CurrentCell != null && Stat.Random(1, Chance) <= 1 && !ParentObject.pBrain.Goals.Items.Any((GoalHandler i) => i.GetType() == typeof(GiveATreatToPartyLeader)))
		{
			ParentObject.pBrain.PushGoal(new GiveATreatToPartyLeader(TreatTable));
