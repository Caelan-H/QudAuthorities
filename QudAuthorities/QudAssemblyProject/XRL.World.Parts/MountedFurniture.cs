...
public class MountedFurniture : IPart
{
	...
	{
		if (!base.WantEvent(ID, cascade) && (ID != AfterObjectCreatedEvent.ID || !Mounted) && (ID != AnimateEvent.ID || !Mounted) && ID != ApplyEffectEvent.ID && ID != CanApplyEffectEvent.ID && ID != EffectAppliedEvent.ID && ID != EquipperEquippedEvent.ID && ID != GetDebugInternalsEvent.ID && ID != GetKineticResistanceEvent.ID && ID != IsRootedInPlaceEvent.ID)
		{
	...
	{
		if (Mounted && E.Item?.GetPart("Armor") is Armor armor && (armor.WornOn == "Feet" || armor.WornOn == "Roots"))
		{
			Mounted = false;
		}
		return base.HandleEvent(E);
