	...
	public override bool FireEvent(Event E)
	{
		if (E.ID == "EnteredCell")
		{
			if (SpawnAsDropColor)
			{
				...
				ParentObject.pRender.DetailColor = gameObject.pRender.GetForegroundColor();
			}
		}
		else if (E.ID == "BeforeTookDamage")
		{
			...
			if (Chance.in100() && !string.IsNullOrEmpty(Blueprint))
			{
				...
				for (int num = Number.RollCached(); i < num; i++)
				{
					...
					DidXToY("drop", gameObject2, null, null, null, null, null, UseFullNames: false, IndefiniteSubject: false, IndefiniteObject: true);
				}
			}
		}
