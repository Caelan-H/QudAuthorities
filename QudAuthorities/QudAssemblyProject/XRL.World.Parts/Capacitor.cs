...
public class Capacitor : IRechargeable
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != BeforeDeathRemovalEvent.ID && ID != ChargeAvailableEvent.ID && ID != EffectAppliedEvent.ID && ID != GetDisplayNameEvent.ID && ID != GetInventoryActionsEvent.ID && ID != InventoryActionEvent.ID && (ID != ObjectCreatedEvent.ID || string.IsNullOrEmpty(StartCharge)) && ID != QueryChargeEvent.ID && ID != QueryChargeStorageEvent.ID && ID != QueryRechargeStorageEvent.ID && ID != RechargeAvailableEvent.ID && ID != TestChargeEvent.ID)
		{
	...
	{
		if (MinimumChargeToExplode > 0 && Charge >= MinimumChargeToExplode && (CatastrophicDisable || IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L)))
		{
			DidX("explode", null, "!", null, null, ParentObject);
	...
	{
		if (Charge >= MaxCharge || !IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L) || E.Amount <= 0)
		{
			return;
		}
		int num = E.Amount;
