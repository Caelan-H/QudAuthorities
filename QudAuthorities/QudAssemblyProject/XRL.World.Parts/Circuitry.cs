...
public class Circuitry : IPoweredPart
{
	...
	{
		if (ActiveTurn < XRLCore.CurrentTurn)
		{
			if (ActiveTurn < XRLCore.CurrentTurn - 1)
			{
				Charge = 0;
			}
			else
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != EffectAppliedEvent.ID && ID != FinishChargeAvailableEvent.ID && ID != GetDebugInternalsEvent.ID && ID != GetDisplayNameEvent.ID && ID != QueryChargeEvent.ID && ID != QueryChargeStorageEvent.ID && ID != TestChargeEvent.ID && ID != UseChargeEvent.ID && ID != ZoneActivatedEvent.ID && ID != ZoneDeactivatedEvent.ID)
		{
	...
	{
		if ((!E.LiveOnly || ConsiderLive) && E.IncludeTransient && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L))
		{
			CheckTurn();
	...
	{
		if (E.Pass == 1 && (!E.LiveOnly || ConsiderLive) && E.IncludeTransient && IsReady(UseCharge: false, IgnoreCharge: false, IgnoreBootSequence: false, IgnoreBreakage: false, IgnoreRust: false, IgnoreEMP: false, IgnoreRealityStabilization: false, IgnoreSubject: false, IgnoreLocallyDefinedFailure: false, 1, null, UseChargeIfUnpowered: false, 0L))
		{
			CheckTurn();
			int num = ChargeUse * E.Multiple;
			int num2 = Math.Min(E.Amount, GetCharge() - num);
