...
[UIView("CharacterCreation:BuildLibrary", false, false, false, null, null, false, 0, false, NavCategory = "Menu", UICanvas = "Chargen/BuildLibrary", UICanvasHost = 1)]
public class QudBuildLibraryModuleWindow : EmbarkBuilderModuleWindowPrefabBase<QudBuildLibraryModule, HorizontalScroller>
{
	...
	public async void onSelect(FrameworkDataElement choice)
	{
		...
		{
			QudMenuItem qudMenuItem = await Popup.NewPopupMessageAsync("Paste build code:", PopupMessage.AcceptCancelButton, null, null, "");
			if (qudMenuItem.command != "Cancel")
			{
				await AddBuild(qudMenuItem.text);
	...
	public static async Task AddBuild(string code)
	{
		...
		QudMenuItem qudMenuItem = await Popup.NewPopupMessageAsync("Name this build:", PopupMessage.AcceptCancelButton, null, null, "");
		if (qudMenuItem.command != "Cancel")
		{
			...
			BuildLibrary.AddBuild(code, qudMenuItem.text, tile, foreground, detail);
		}
	}
	...
	public override async void HandleMenuOption(MenuOption menuOption)
	{
		if (menuOption?.Id == "CmdChargenItemOptions" && hasOptions)
		{
			...
			if (qudMenuItem.command != "Cancel")
			{
				...
				{
					qudMenuItem = await Popup.NewPopupMessageAsync("Name this build:", PopupMessage.AcceptCancelButton, null, null, "");
					if (qudMenuItem.command != "Cancel")
					{
						build.Name = qudMenuItem.text;
	...
	public void HighlightBuild(FrameworkDataElement dataElement)
	{
		...
		hasOptions = (dataElement?.Id?.Contains("code:")).GetValueOrDefault();
