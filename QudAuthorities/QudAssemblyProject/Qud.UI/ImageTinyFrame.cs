	...
	public void Sync(bool force = false)
	{
		...
		if (_lastImageSize != imageSize && image != null)
		{
			if (totalSize.magnitude < 1f)
			{
				...
				Vector2 vector2 = (totalSize = (_rectTransform.sizeDelta = imageSize + new Vector2(16f, 14f)));
			}
		...
		if (useImageSwap)
		{
			bool valueOrDefault = (GetComponentInParent<FrameworkContext>()?.context?.IsActive()).GetValueOrDefault();
		...
		{
			bool valueOrDefault2 = (GetComponentInParent<FrameworkContext>()?.context?.IsActive()).GetValueOrDefault();
