...
public abstract class QudBaseMenuController : MonoBehaviour
{
	...
	public virtual void SetSelectedInput(ControlledInputField item, MenuEventPipeline pipeline = MenuEventPipeline.Pointer)
	{
	}

	public virtual void ActivateInput(ControlledInputField item, MenuEventPipeline pipeline = MenuEventPipeline.Pointer)
	{
	}
}
public abstract class QudBaseMenuController<DataT, UnityT> : QudBaseMenuController where UnityT : ControlledSelectable
{
	...
	public List<ControlledInputField> inputFields;
	...
	public IEnumerable<IControlledSelectable> GetOptions()
	{
		if (inputFields != null)
		{
			foreach (ControlledInputField inputField in inputFields)
			{
	...
	public virtual void Update()
	{
		...
		if (ControlManager.isKeyDown(UnityEngine.KeyCode.PageDown))
		{
		...
		if (ControlManager.isKeyDown(UnityEngine.KeyCode.PageUp))
		{
	...
	public virtual void CheckKeyInteractions(IEnumerable<QudMenuItem> items, Action<QudMenuItem, MenuEventPipeline> activateHandler)
	{
		...
		{
			string[] array = (item.hotkey ?? "").Split(',');
			foreach (string text in array)
			{
				...
				case "Accept":
				case "CmdHelp":
					if (ControlManager.isCommandDown(text))
					{
				...
				try
				{
					if (ControlManager.isKeyDown(Keyboard.ParseUnityEngineKeyCode(text)))
					{
