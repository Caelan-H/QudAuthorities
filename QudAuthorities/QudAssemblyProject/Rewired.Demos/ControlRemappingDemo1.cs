...
public class ControlRemappingDemo1 : MonoBehaviour
{
	...
	{
		if (!dialog.enabled && Event.current.type == EventType.Layout && ReInput.players.Players[0].GetButtonDown("Accept"))
		{
			if (showMenu)
			{
	...
	{
		ActionElementMap firstElementMapWithAction = ReInput.players.Players[0].controllers.maps.GetFirstElementMapWithAction("Accept", skipDisabledMaps: true);
