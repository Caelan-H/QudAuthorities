...
public class exUIScrollView : exUIControl
{
	...
	protected new void Awake()
	{
		...
		AddEventListener("onPressUp", delegate(exUIEvent _event)
		{
			...
			if (draggable && (exUIPointEvent3.isTouch || exUIPointEvent3.GetMouseButton(0)) && exUIPointEvent3.pointInfos[0].id == draggingID)
			{
				if (dragging)
				{
					dragging = false;
		...
		AddEventListener("onHoverMove", delegate(exUIEvent _event)
		{
			...
			else
			{
				...
				Rect bound = new Rect(x2, y2, Mathf.Max(contentSize_.x, width), Mathf.Max(contentSize_.y, height));
				...
				for (int i = 0; i < exUIPointEvent2.pointInfos.Length; i++)
				{
					...
					{
						Vector2 vector2 = exUIPointInfo2.worldDelta;
