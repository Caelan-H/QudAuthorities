...
public class ConveyorBelt
{
	...
	public bool BuildZone(Zone Z)
	{
		...
		if (StepDirection != "")
		{
			...
			while ((x != x2 || y != y2) && cell != null)
			{
				...
				for (int i = 0; i < StepDirection.Length; i++)
				{
					...
					if (cell2 != null && !cell2.HasObjectWithIntProperty("Wall") && !cell2.HasObjectWithPart("ConveyorBelt"))
					{
						break;
					}
					cell2 = null;
		...
		foreach (Cell step in findPath.Steps)
		{
			...
			ConveyorPad conveyorPad = gameObject.GetPart("ConveyorPad") as ConveyorPad;
			if (num < findPath.Steps.Count - 1)
			{
				conveyorPad.Direction = step.GetDirectionFromCell(findPath.Steps[num + 1]);
				conveyorPad.Connections = conveyorPad.Direction;
				text = conveyorPad.Direction;
			}
			else
			{
				conveyorPad.Direction = text;
				conveyorPad.Connections = text;
			}
