...
public class MechanicalPower : ZoneBuilderSandbox
{
	...
	private bool IsProducer(GameObject obj)
	{
		...
		if (obj.GetPart("MechanicalPowerTransmission") is MechanicalPowerTransmission mechanicalPowerTransmission && mechanicalPowerTransmission.IsProducer)
		{
			return true;
		}
	...
	private bool IsConsumer(GameObject obj)
	{
		...
		if (obj.GetPart("MechanicalPowerTransmission") is MechanicalPowerTransmission mechanicalPowerTransmission && mechanicalPowerTransmission.IsConsumer)
		{
			return true;
		}
	...
	public bool BuildZone(Zone Z)
	{
		...
		for (int num = 0; num < Z.Height; num++)
		{
			for (int num2 = 0; num2 < Z.Width; num2++)
			{
				...
				{
					if (gameObject.GetPart("MechanicalPowerTransmission") is MechanicalPowerTransmission mechanicalPowerTransmission)
					{
						num3 -= 20 + mechanicalPowerTransmission.ChargeRate / 100;
					}
					else if (AvoidWalls)
					{
				...
				{
					if (gameObject2.GetPart("MechanicalPowerTransmission") is MechanicalPowerTransmission mechanicalPowerTransmission2)
					{
						num3 -= 18 + mechanicalPowerTransmission2.ChargeRate / 100;
					}
					else if (AvoidWalls)
					{
				...
				{
					num3 = ((!(gameObject5.GetPart("MechanicalPowerTransmission") is MechanicalPowerTransmission mechanicalPowerTransmission3)) ? (num3 - 22) : (num3 - (20 + mechanicalPowerTransmission3.ChargeRate / 100)));
				}
			...
			foreach (GameObject consumer in consumers)
			{
				...
				for (int num13 = pathfinder.Steps.Count - 1; num12 < num13; num12++)
				{
					...
					if (flag2)
					{
						...
						MechanicalPowerTransmission mechanicalPowerTransmission4 = gameObject7?.GetPart<MechanicalPowerTransmission>() ?? gameObject8?.GetPart<MechanicalPowerTransmission>();
						weight -= ((mechanicalPowerTransmission4 == null) ? 12 : (10 + mechanicalPowerTransmission4.ChargeRate / 500));
						if (weight < 1)
						{
							weight = 1;
						}
