	...
	public bool BuildZone(Zone Z)
	{
		...
		for (int i = 0; i < Z.Width; i++)
		{
			for (int j = 0; j < Z.Height; j++)
			{
				...
				{
					double num3 = CaveNoise[i + num, j + num2];
					if (cellularGrid.cells[i, j] == 0 || num3 <= 0.4699999988079071)
					{
						Z.GetCell(i, j).Clear();
						Z.GetCell(i, j).SetReachable(State: true);
					}
				}
			}
		}
		...
		for (int m = 0; m < Z.Width; m++)
		{
			...
			while (num6 < Z.Height)
			{
				if (!Z.GetCell(m, num6).IsReachable())
				{
					num6++;
					continue;
				}
			...
			continue;
			IL_08ce:
			Z.BuildReachableMap(m, num6, bClearFirst: false);
			break;
		}
		if (Z.Z > 49)
		{
