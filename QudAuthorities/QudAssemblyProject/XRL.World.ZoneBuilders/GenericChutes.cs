...
public class GenericChutes
{
	...
	public bool BuildZone(Zone Z)
	{
		...
		foreach (Box chute in genericChuteTemplate.Chutes)
		{
			...
			XRLCore.Core.Game.ZoneManager.SetZoneProperty(Z.ZoneID, "Chute" + num + "StartY", text);
			XRLCore.Core.Game.ZoneManager.SetZoneProperty(Z.ZoneID, "Chute" + num + "EndY", text2);
			TunnelMaker tunnelMaker = ((Z.Z % 2 != 0) ? new TunnelMaker(num2, num3, text, text2, "WNS") : new TunnelMaker(num2, num3, text, text2, "ENS"));
			bool flag = Z.Z % 2 == 0;
			if (chuteLevel)
			{
				for (int i = 0; i < tunnelMaker.Width; i++)
				{
					for (int j = 0; j < tunnelMaker.Height; j++)
					{
						int num4 = i * 4 + chute.x1 + 1;
						int num5 = j * 3 + chute.y1 + 1;
						string text3 = tunnelMaker.Map[i, j];
						switch (text3)
						{
						case "N":
						case "S":
						case "E":
						case "W":
							text3 = (((!flag || i != 0) && (flag || i != tunnelMaker.Width - 1)) ? (text3 + "END") : (text3 + "START"));
							break;
						}
						...
						for (int k = 0; k < 4; k++)
						{
							for (int l = 0; l < 3; l++)
							{
								...
								if (Z.Z > 10)
								{
									...
									ConveyorPad obj = gameObject.GetPart("ConveyorPad") as ConveyorPad;
									obj.Direction = c.ToString();
									obj.Connections = c.ToString();
				...
				if (Z.Z > 10)
				{
					...
					for (; m < 100; m++)
					{
						...
						if (!Z.GetCell(x, y).HasObjectWithIntProperty("Wall"))
						{
							continue;
						}
						bool flag2 = false;
						...
						if (flag2)
						{
							...
							if (num == 1)
							{
								...
								Walltrap walltrap2 = gameObject2.GetPart("Walltrap") as Walltrap;
								walltrap2.TurnInterval = Stat.Random(12, 16);
								walltrap2.CurrentTurn = Stat.Random(0, walltrap2.TurnInterval - 2);
							}
							if (num == 2)
							{
								gameObject2 = GameObjectFactory.Factory.CreateObject("WalltrapShock");
								Walltrap walltrap3 = gameObject2.GetPart("Walltrap") as Walltrap;
								walltrap3.TurnInterval = Stat.Random(4, 8);
								walltrap3.CurrentTurn = Stat.Random(0, walltrap3.TurnInterval - 2);
							}
							if (num == 3)
							{
								gameObject2 = GameObjectFactory.Factory.CreateObject("WalltrapCrabs");
								Walltrap walltrap4 = gameObject2.GetPart("Walltrap") as Walltrap;
								walltrap4.TurnInterval = Stat.Random(4, 16);
								walltrap4.CurrentTurn = Stat.Random(0, walltrap4.TurnInterval - 2);
							}
			...
			{
				for (int n = 0; n < tunnelMaker.Width; n++)
				{
					for (int num8 = 0; num8 < tunnelMaker.Height; num8++)
					{
						int num9 = n * 4 + chute.x1 + 1;
						int num10 = num8 * 3 + chute.y1 + 1;
						string text4 = tunnelMaker.Map[n, num8];
						switch (text4)
						{
						case "N":
						case "S":
						case "E":
						case "W":
							text4 = (((!flag || n != 0) && (flag || n != tunnelMaker.Width - 1)) ? (text4 + "END") : (text4 + "START"));
							break;
						}
