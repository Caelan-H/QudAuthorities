...
public class BuildingZoneTemplate
{
	...
	public Point2D GetRandomWall()
	{
		...
		if (num > 0)
		{
			...
			for (int k = 0; k < Width; k++)
			{
				...
				{
					if (Template.Map[k, l] == BuildingTemplateTile.Wall && ++num3 == num2)
					{
						return new Point2D(k, l);
					}
				}
			}
		}
	...
	public BuildingTemplate FromWFC(string wfcTemplateTable, string segmentation, int width, int height, int hmirrorchance = 20, int vmirrorchance = 20)
	{
			...
			case "BlocksAndCircles":
			{
				...
				for (int l = 0; l < num6; l++)
				{
					...
					for (int m = 0; m <= width; m++)
					{
						...
						{
							if (m <= 0 || n <= 0 || m >= width || n >= height || CosmeticDistanceTo(m, n, x2, y2) > num7)
							{
								continue;
							}
							if (CosmeticDistanceTo(m, n, x2, y2) == num7 || (CosmeticDistanceTo(m, n, x2, y2) <= num7 && m == 0) || n == 0 || m == width - 1 || n == height - 1)
							{
								if (grid2.get(m, n) == Color4.red)
								{
									grid2.set(m, n, Color4.black);
								}
							}
							else
							{
								grid2.set(m, n, Color4.white);
							}
						}
					}
				}
		...
		if (Stat.Random(1, 100) <= hmirrorchance)
		{
			grid2 = grid2.mirrorHorizontal();
		}
		if (Stat.Random(1, 100) <= vmirrorchance)
		{
		...
		for (int num8 = 0; num8 < width; num8++)
		{
			...
			{
				if (grid2.get(num8, num9) == Color4.red)
				{
					buildingTemplate.Map[num8, num9] = BuildingTemplateTile.Outside;
				}
				else if (grid.get(num8, num9) == Color4.black || grid2.get(num8, num9) == Color4.black)
				{
					buildingTemplate.Map[num8, num9] = BuildingTemplateTile.Wall;
				}
				else if (grid.get(num8, num9) == Color4.white)
				{
					buildingTemplate.Map[num8, num9] = BuildingTemplateTile.Outside;
				}
				else
				{
					buildingTemplate.Map[num8, num9] = BuildingTemplateTile.Inside;
				}
			}
		}
	...
	public void BuildZone(Zone Z, bool bUnderground)
	{
		...
		for (int i = 0; i < Z.Width; i++)
		{
			for (int j = 0; j < Z.Height; j++)
			{
				...
				if (i >= Template.Width || j >= Template.Height)
				{
					cell.Clear(null, Important: false, Combat: false, (GameObject go) => go.HasTag("NoRuinsRemove"));
				}
				else if (Template.Map[i, j] == BuildingTemplateTile.OutsideWall || Template.Map[i, j] == BuildingTemplateTile.Wall)
				{
					cell.Clear(null, Important: false, Combat: false, (GameObject go) => go.HasTag("NoRuinsRemove"));
					cell.AddObject(Value);
				}
				else if (Template.Map[i, j] == BuildingTemplateTile.Door)
				{
					cell.Clear(null, Important: false, Combat: false, (GameObject go) => go.HasTag("NoRuinsRemove"));
					cell.AddObject("Door");
				}
				else if (Template.Map[i, j] == BuildingTemplateTile.StairsDown)
				{
					cell.Clear();
					if (Z.GetZoneZ() != 10)
					{
				...
				else if (Template.Map[i, j] == BuildingTemplateTile.StairsUp)
				{
					cell.Clear();
					if (Z.GetZoneZ() % 2 == 0)
					{
				...
				else if ((!bUnderground || Template.Map[i, j] != BuildingTemplateTile.Outside) && !bUnderground && Template.Map[i, j] == BuildingTemplateTile.Inside && !cell.HasObjectWithTag("NoRuinsRemove"))
				{
					cell.ClearWalls();
				}
			}
