...
public class ForceConnections : ZoneBuilderSandbox
{
	...
	public bool _BuildZone(Zone Z, List<Point2D> additional = null)
	{
		...
		{
			if (Z.IsReachable(list[k].x, list[k].y) || !pathfinder.FindPath(list[0].location, list[k].location, bDisplay: false, bOrdinalDirectionsOnly: true, 24300, shuffleDirections: true))
			{
				continue;
			}
			...
			if (CaveLike)
			{
				foreach (PathfinderNode step2 in pathfinder.Steps)
				{
					...
					if (cell == null || cell.IsReachable() || cell.HasObjectWithTag("Wall"))
					{
						continue;
					}
