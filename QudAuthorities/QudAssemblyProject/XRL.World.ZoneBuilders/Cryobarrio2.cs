	...
	public static void MakeCryochamber(int x, int y, Zone Z, string Contents = null, string Plaque = null)
	{
		...
		if (!string.IsNullOrEmpty(Contents) && Contents != "*Destroyed")
		{
			...
			if (Contents == "*Clone")
			{
				if (The.Player != null)
				{
					gameObject = Cloning.GenerateClone(The.Player, null, null, DuplicateGear: true, BecomesCompanion: false, Budded: false, "Cryotube", ItemNaming.GetPreNamingVersion);
				}
			}
			else
			{
