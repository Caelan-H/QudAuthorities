...
public class PowerGrid : ZoneBuilderSandbox
{
	...
	private bool IsProducer(GameObject obj)
	{
		...
		if (obj.GetPart("ElectricalPowerTransmission") is ElectricalPowerTransmission electricalPowerTransmission && electricalPowerTransmission.IsProducer)
		{
			return true;
		}
	...
	private bool IsConsumer(GameObject obj)
	{
		...
		if (obj.GetPart("ElectricalPowerTransmission") is ElectricalPowerTransmission electricalPowerTransmission && electricalPowerTransmission.IsConsumer)
		{
			return true;
		}
		if (obj.HasTagOrProperty("GridPowerConsumer"))
	...
	private bool ValidConduitInstall(GameObject obj)
	{
		...
		return TechModding.ModificationApplicable(ConduitModPart, obj);
	}
	...
	public bool BuildZone(Zone Z)
	{
		...
		for (int num = 0; num < Z.Height; num++)
		{
			for (int num2 = 0; num2 < Z.Width; num2++)
			{
				...
				{
					if (gameObject.GetPart("ElectricalPowerTransmission") is ElectricalPowerTransmission electricalPowerTransmission)
					{
						num3 -= 20 + electricalPowerTransmission.ChargeRate / 250;
					}
					else if (AvoidWalls)
					{
				...
				{
					if (gameObject2.GetPart("ElectricalPowerTransmission") is ElectricalPowerTransmission electricalPowerTransmission2)
					{
						num3 -= 18 + electricalPowerTransmission2.ChargeRate / 250;
					}
					if (AvoidWalls)
					{
				...
				{
					num3 = ((!(gameObject5.GetPart("ElectricalPowerTransmission") is ElectricalPowerTransmission electricalPowerTransmission3)) ? (num3 - 22) : (num3 - (20 + electricalPowerTransmission3.ChargeRate / 250)));
				}
			...
			foreach (GameObject consumer in consumers)
			{
				...
				for (int num13 = pathfinder.Steps.Count - 1; num12 < num13; num12++)
				{
					...
					if (flag2)
					{
						...
						ElectricalPowerTransmission electricalPowerTransmission4 = gameObject7?.GetPart<ElectricalPowerTransmission>() ?? gameObject8?.GetPart<ElectricalPowerTransmission>();
						weight -= ((electricalPowerTransmission4 == null) ? 12 : (10 + electricalPowerTransmission4.ChargeRate / 500));
						if (weight < 1)
						{
							weight = 1;
						}
