...
public class SpireTemplate
{
	...
	public SpireTemplate(int _Width, int _Height, int nSquares)
	{
		...
		for (int i = 0; i < Width; i++)
		{
			for (int j = 0; j < Height; j++)
			{
				Map[i, j] = SpireTemplateTile.Void;
				Rooms[i, j] = 0;
			}
		}
	...
	public void GenerateRooms(int Depth)
	{
		...
		for (int i = 0; i < Width; i++)
		{
			for (int j = 0; j < Height; j++)
			{
				if (Map[i, j] == SpireTemplateTile.OutsideWall)
				{
					Map[i, j] = SpireTemplateTile.Wall;
				}
			}
		}
		for (int k = 0; k < Width; k++)
		{
			...
			{
				if (Map[k, l] == SpireTemplateTile.Inside)
				{
					FillRoom(k, l, 0);
					break;
				}
		...
		int num = 0;
		for (short num2 = 0; num2 < Width; num2 = (short)(num2 + 1))
		{
			for (short num3 = 0; num3 < Height; num3 = (short)(num3 + 1))
			{
	...
	public void Draw(SpireRoomData Data)
	{
		...
		for (int i = 0; i < Width; i++)
		{
			for (int j = 0; j < Height; j++)
			{
				if (Map[i, j] == SpireTemplateTile.Inside)
				{
					Popup._ScreenBuffer.Goto(i, j);
					Popup._ScreenBuffer.Write(58);
				}
				if (Map[i, j] == SpireTemplateTile.Void)
				{
					Popup._ScreenBuffer.Goto(i, j);
					Popup._ScreenBuffer.Write(88);
				}
				if (Map[i, j] == SpireTemplateTile.Outside)
				{
					Popup._ScreenBuffer.Goto(i, j);
				...
				{
					Popup._ScreenBuffer.Goto(i, j);
					Popup._ScreenBuffer.Write((short)(65 + Data.Room[i, j]));
				}
				if (Map[i, j] == SpireTemplateTile.Wall)
				{
					Popup._ScreenBuffer.Goto(i, j);
					Popup._ScreenBuffer.Write(42);
				}
				if (Map[i, j] == SpireTemplateTile.Door)
				{
					Popup._ScreenBuffer.Goto(i, j);
