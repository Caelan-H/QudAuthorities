...
public class BeyLahOutskirts : ZoneBuilderSandbox
{
	public bool BuildZone(Zone Z)
	{
		...
		for (int i = 0; i < Z.Width; i++)
		{
			...
			{
				if (noiseMap.Noise[i, j] > 3 && Stat.Random(1, 100) <= 90)
				{
					Z.GetCell(i, j).AddObject(PopulationManager.RollOneFrom("BeyLahOutskirtsTrees").Blueprint);
				}
			}
		}
		...
		for (int n = 0; n < Stat.Random(0, 4); n++)
		{
