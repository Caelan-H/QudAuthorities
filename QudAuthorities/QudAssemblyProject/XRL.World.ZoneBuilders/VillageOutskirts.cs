	...
	public bool BuildZone(Zone Z)
	{
		...
		for (int num9 = 0; num9 < zone.Width; num9++)
		{
			for (int num10 = 0; num10 < zone.Height; num10++)
			{
				...
				{
					pathfinder.CurrentNavigationMap[num9, num10] = 999;
				}
				else
				{
		...
		foreach (PopulationLayout building3 in buildings)
		{
			foreach (Location2D cell3 in building3.region.Cells)
			{
				...
				if (x != 0 && x != 79 && y != 0 && y != 24 && Z.GetCell(x, y).IsEmpty())
				{
					int num11 = 0;
					int num12 = 0;
					if (Z.GetCell(x - 1, y).HasObjectWithTag("Wall") || Z.GetCell(x - 1, y).HasObjectWithTag("Door"))
					{
						num12++;
					}
					if (Z.GetCell(x + 1, y).HasObjectWithTag("Wall") || Z.GetCell(x + 1, y).HasObjectWithTag("Door"))
					...
					if (Z.GetCell(x, y - 1).HasObjectWithTag("Wall") || Z.GetCell(x, y - 1).HasObjectWithTag("Door"))
					{
						num11++;
					}
					if (Z.GetCell(x, y + 1).HasObjectWithTag("Wall") || Z.GetCell(x, y + 1).HasObjectWithTag("Door"))
		...
		else
		{
			...
			{
				if (o.GetBlueprint().InheritsFrom("Furniture") && o.pPhysics != null)
				{
