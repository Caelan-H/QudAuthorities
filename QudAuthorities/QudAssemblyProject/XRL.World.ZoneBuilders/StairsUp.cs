...
public class StairsUp
{
	...
	[NonSerialized]
	private static string[] StairsDownBlueprints = new string[4] { "StairsDown", "OpenShaft", "Pit", "StairBlocker" };
	...
	public static bool AddStairUp(Zone Z, string x, string y, bool Reachable)
	{
		...
		foreach (Cell item2 in list)
		{
			...
			int startX = item2.X;
			int startY = item2.Y;
			if (!item2.HasObjectWithBlueprint("StairsDown") && !item2.HasObjectWithBlueprint("StairsUp") && !item2.HasObjectWithBlueprint("OpenShaft") && !item2.HasObjectWithBlueprint("Pit") && item2.IsReachable())
			{
				item2.Clear();
