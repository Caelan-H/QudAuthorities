...
public class VillageMaker : ZoneBuilderSandbox
{
	public bool BuildZone(Zone Z, bool bRoads, string WallObject, bool RoundBuildings, string Huts, string Features, string HutTable, string ZoneTable, string Widgets, bool ClearCombatObjectsFirst)
	{
		if (ClearCombatObjectsFirst)
		{
			for (int i = 0; i < Z.Width; i++)
			{
				for (int j = 0; j < Z.Height; j++)
				{
					while (true)
					{
						using (List<GameObject>.Enumerator enumerator = Z.GetCell(i, j).GetObjectsWithPart("Combat").GetEnumerator())
						{
		...
		for (int num7 = 0; num7 < Z.Width; num7++)
		{
			...
			{
				if (!Z.Map[num7][num8].HasObject("DirtRoad"))
				{
					continue;
				}
				foreach (Cell localAdjacentCell in Z.Map[num7][num8].GetLocalAdjacentCells())
				{
