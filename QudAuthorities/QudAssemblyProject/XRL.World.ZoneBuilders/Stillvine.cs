...
public class Stillvine : ZoneBuilderSandbox
{
	...
	public bool BuildZone(Zone Z)
	{
		...
		for (int k = 0; k < Z.Width; k++)
		{
			...
			{
				if (Underground && !Z.GetCell(k, l).IsPassable())
				{
					continue;
				}
				double num3 = (double)WatervineNoise[k + num, l + num2] + array[k, l];
				if (noiseMap.Noise[k, l] > 3)
				{
					Z.GetCell(k, l).AddObject(GameObjectFactory.Factory.CreateObject("SaltyWaterPuddle"));
				}
				else if (num3 >= 0.7)
				{
					Z.GetCell(k, l).AddObject(GameObjectFactory.Factory.CreateObject("SaltyWaterPuddle"));
				...
				else if (num3 >= 0.6)
				{
					...
					{
						Z.GetCell(k, l).AddObject(GameObjectFactory.Factory.CreateObject("Stillvine"));
					}
				}
				else
				{
					Z.GetCell(k, l).SetReachable(State: true);
				}
			}
		}
		if (!Underground)
