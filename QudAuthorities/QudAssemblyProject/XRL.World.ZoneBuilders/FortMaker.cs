...
public class FortMaker : ZoneBuilderSandbox
{
	...
	{
		List<Box> list = Tools.GenerateBoxes(BoxGenerateOverlap.Irrelevant, new Range(1, 1), new Range(30, 50), new Range(16, 24), null);
		if (list == null)
		{
			return false;
		}
		...
		if (ClearCombatObjectsFirst)
		{
			for (int i = box.x1; i <= box.x2; i++)
			{
				for (int j = box.y1; j <= box.y2; j++)
				{
					while (true)
					{
						using (List<GameObject>.Enumerator enumerator = Z.GetCell(i, j).GetObjectsWithPartReadonly("Combat").GetEnumerator())
						{
							if (enumerator.MoveNext())
							{
								GameObject current = enumerator.Current;
								Z.GetCell(i, j).RemoveObject(current);
								continue;
							}
						}
						break;
		...
		foreach (Box item2 in list2)
		{
			...
			while (num3 < 1000)
			{
				...
				if (num4 != box.x1 && num4 != box.x2 && num5 != box.y1 && num5 != box.y2 && ((!Z.GetCell(num4 - 1, num5).HasObjectWithIntProperty("Wall") && !Z.GetCell(num4 + 1, num5).HasObjectWithIntProperty("Wall")) || (!Z.GetCell(num4, num5 - 1).HasObjectWithIntProperty("Wall") && !Z.GetCell(num4, num5 + 1).HasObjectWithIntProperty("Wall"))))
				{
		...
		if (((uint)num7 & (true ? 1u : 0u)) != 0)
		{
			...
			if (!Z.GetCell(num8, box.y1 + 1).HasObjectWithIntProperty("Wall"))
			{
				Z.GetCell(num8, box.y1).Clear();
				Z.GetCell(num8, box.y1).AddObject("Door");
			}
			if (!Z.GetCell(num8 + 1, box.y1 + 1).HasObjectWithIntProperty("Wall"))
			{
		...
		if (((uint)num7 & 2u) != 0)
		{
			...
			if (!Z.GetCell(num9, box.y2 - 1).HasObjectWithIntProperty("Wall"))
			{
				Z.GetCell(num9, box.y2).Clear();
				Z.GetCell(num9, box.y2).AddObject("Door");
			}
			if (!Z.GetCell(num9 + 1, box.y2 - 1).HasObjectWithIntProperty("Wall"))
			{
		...
		if (((uint)num7 & 4u) != 0)
		{
			...
			if (!Z.GetCell(box.x1 + 1, num10).HasObjectWithIntProperty("Wall"))
			{
				Z.GetCell(box.x1, num10).Clear();
				Z.GetCell(box.x1, num10).AddObject("Door");
			}
			if (!Z.GetCell(box.x1 + 1, num10 + 1).HasObjectWithIntProperty("Wall"))
			{
		...
		if (((uint)num7 & 8u) != 0)
		{
			...
			if (!Z.GetCell(box.x2 - 1, num11).HasObjectWithIntProperty("Wall"))
			{
				Z.GetCell(box.x2, num11).Clear();
				Z.GetCell(box.x2, num11).AddObject("Door");
			}
			if (!Z.GetCell(box.x2 - 1, num11 + 1).HasObjectWithIntProperty("Wall"))
			{
