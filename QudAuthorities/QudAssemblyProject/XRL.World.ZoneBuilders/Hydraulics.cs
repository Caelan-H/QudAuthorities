...
public class Hydraulics : ZoneBuilderSandbox
{
	...
	private bool IsProducer(GameObject obj)
	{
		...
		if (obj.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission && hydraulicPowerTransmission.IsProducer)
		{
			return true;
		}
	...
	private bool IsConsumer(GameObject obj)
	{
		...
		if (obj.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission && hydraulicPowerTransmission.IsConsumer)
		{
			return true;
		}
		if (obj.HasTagOrProperty("HydraulicPowerConsumer"))
	...
	private GameObject SetUpMissingConsumer(Zone Z)
	{
		...
		if (missingConsumerIndex >= missingConsumers.Count)
		{
			...
			gameObject.pPhysics.Solid = true;
	...
	public bool BuildZone(Zone Z)
	{
		...
		Pathfinder pathfinder = new Pathfinder(Z.Width, Z.Height);
		...
		for (int num = 0; num < Z.Height; num++)
		{
			for (int num2 = 0; num2 < Z.Width; num2++)
			{
				...
				{
					if (gameObject == null && @object.HasTagOrProperty("Wall") && !@object.HasPart("ExistenceSupport"))
					{
						gameObject = @object;
					}
				...
				if (gameObject3 != null || gameObject4 != null)
				{
					...
					if (gameObject == null && PreferWalls && !AvoidWalls)
					{
						...
						{
							if (localCardinalAdjacentCell.HasObjectWithTagOrProperty("Wall"))
							{
				...
				{
					if (gameObject.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission)
					{
						num3 -= 20 + hydraulicPowerTransmission.ChargeRate / 1000;
				...
				{
					if (gameObject2.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission2)
					{
						num3 -= 18 + hydraulicPowerTransmission2.ChargeRate / 1000;
				...
				{
					if (gameObject5.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission3)
					{
						conduits.Add(gameObject5);
						num3 -= 20 + hydraulicPowerTransmission3.ChargeRate / 1000;
					}
					else
					{
						num3 -= 22;
		...
		if (producers.Count > 0 && consumers.Count > 0)
		{
			...
			foreach (GameObject consumer in consumers)
			{
				...
				if (!pathfinder.FindPath(location2D, location2D2, ShowPathfinding, bOrdinalDirectionsOnly: true))
				{
					continue;
				}
				int num12 = 1;
				for (int num13 = pathfinder.Steps.Count - 1; num12 < num13; num12++)
				{
					...
					if (gameObject7 != null)
					{
						...
						{
							TechModding.ApplyModification(gameObject7, ConduitModPart);
					...
					if (flag2)
					{
						...
						HydraulicPowerTransmission hydraulicPowerTransmission4 = gameObject7?.GetPart<HydraulicPowerTransmission>() ?? gameObject8?.GetPart<HydraulicPowerTransmission>();
						weight -= ((hydraulicPowerTransmission4 == null) ? 12 : (10 + hydraulicPowerTransmission4.ChargeRate / 500));
						if (weight < 1)
						{
							weight = 1;
						}
		...
		if (!string.IsNullOrEmpty(Liquid))
		{
			...
			{
				if (item.GetPart("HydraulicPowerTransmission") is HydraulicPowerTransmission hydraulicPowerTransmission5 && !string.IsNullOrEmpty(hydraulicPowerTransmission5.DependsOnLiquid))
				{
					hydraulicPowerTransmission5.DependsOnLiquid = text;
