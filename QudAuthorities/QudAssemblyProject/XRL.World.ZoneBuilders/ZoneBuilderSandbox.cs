...
public class ZoneBuilderSandbox
{
	...
	public void EnsureCellReachable(Zone zone, Cell cell)
	{
		...
		if (!pathfinder.FindPath(cell.location, list[0].location, bDisplay: false, bOrdinalDirectionsOnly: true, 24300))
		{
			return;
		}
	...
	{
		Pathfinder pathfinder = new Pathfinder(80, 25);
		if (pathWithNoise)
		{
		...
		if (!pathfinder.FindPath(start, end, bDisplay: false, bOrdinalDirectionsOnly: true, 24300, shuffleDirections: true))
		{
			return;
		}
		foreach (PathfinderNode step in pathfinder.Steps)
	...
	public static void EnsureAllVoidsConnected(Zone zone, bool pathWithNoise = false, float pathWindyness = 0.2f)
	{
		Grid<int> grid = new Grid<int>(zone.Width, zone.Height);
		InfluenceMap regions = grid.regionalize(delegate(int x, int y, int c)
		{
			...
			return zone.GetCell(x, y).HasObject((GameObject o) => o.pPhysics != null && o.pPhysics.Solid) ? 80 : 0;
		});
		...
		foreach (InfluenceMapRegion region in regions.Regions)
		{
			if (list.Count > 0)
			{
				...
				pathfinder.setWeightsFromGrid(grid, delegate(int x, int y, int c)
				{
					...
					return zone.GetCell(x, y).HasObject((GameObject o) => o.pPhysics != null && o.pPhysics.Solid) ? 1000 : num;
				});
				if (!pathfinder.FindPath(randomElement, randomElement2, bDisplay: false, bOrdinalDirectionsOnly: true, 24300, shuffleDirections: true))
				{
					continue;
				}
				foreach (PathfinderNode step in pathfinder.Steps)
				{
					...
					cell.ClearWalls();
					if (cell.HasObject((GameObject o) => o.pPhysics != null && o.pPhysics.Solid))
					{
						GameObject firstObject = cell.GetFirstObject((GameObject o) => o.pPhysics != null && o.pPhysics.Solid);
	...
	public List<Location2D> CreateConveyor(Location2D start, Location2D end, Zone zone, Func<int, int, int> getExtraWeight = null, string forceFinalDirection = null, string padObject = "ConveyorPad")
	{
		...
		{
			int num2 = ((getExtraWeight != null) ? getExtraWeight(x, y) : 0);
			if (cell.HasObject("ConveyorPad") || cell.HasObject(padObject) || cell.HasObject("ConveyorDrive"))
			{
				return int.MaxValue;
			}
			return cell.HasWall() ? (40 + num2) : num2;
		});
		int num = 10;
		while (true)
		{
			pathfinder.FindPath(start, end, bDisplay: false, bOrdinalDirectionsOnly: true, 9999, shuffleDirections: true);
		...
		for (int i = 0; i < pathfinder.Steps.Count; i++)
		{
			...
			if (gameObject.GetPart("ConveyorPad") is ConveyorPad conveyorPad)
			{
				conveyorPad.Connections = (conveyorPad.Direction = ((i < pathfinder.Steps.Count - 1) ? pathfinder.Directions[i] : ((forceFinalDirection == null) ? pathfinder.Directions[i - 1] : forceFinalDirection)));
				if (i > 0)
				{
					conveyorPad.Connections += Directions.GetOppositeDirection(pathfinder.Directions[i - 1]);
				}
			}
			else
			{
	...
	{
		return new System.Random(Hash.String(Z.wX + "." + Z.wY + "." + Z.X + "." + Z.Y + seed)).Next(Low, High);
	}

	public static int GetOracleIntColumn(Zone Z, int Low, int High)
	{
		return new System.Random(Hash.String(Z.wX + "." + Z.wY + "." + Z.X + "." + Z.Y)).Next(Low, High);
	}
	...
	public List<Location2D> BuildPathWithObject(Zone zone, List<Location2D> Segments, string PaintObject, int Width, bool bNoise, Func<int, int, int, int> weightFunc = null)
	{
		...
		{
			pathfinder.FindPath(Segments[i], Segments[i + 1], bDisplay: false, bOrdinalDirectionsOnly: true);
			if (!pathfinder.bFound)
			{
				continue;
			}
			foreach (PathfinderNode step in pathfinder.Steps)
	...
	public List<Location2D> BuildSimplePathWithObject(Zone Z, List<Location2D> Segments, string Blueprint, int Width, bool bNoise)
	{
		List<Location2D> list = new List<Location2D>();
		GameObject looker = null;
		for (int i = 0; i < Segments.Count; i += 2)
		{
			FindPath findPath = new FindPath(Z, Segments[i].x, Segments[i].y, Z, Segments[i + 1].x, Segments[i + 1].y, PathGlobal: false, PathUnlimited: true, looker, bNoise);
			if (!findPath.bFound)
			{
				continue;
			}
	...
	public List<Location2D> BuildPath(Zone Z, List<Location2D> Segments, string PaintTile, string PaintRender, string PaintColor, string PaintDetail, int Width, bool bNoise)
	{
		List<Location2D> list = new List<Location2D>();
		GameObject looker = null;
		for (int i = 0; i < Segments.Count; i += 2)
	...
	public static void encloseRectWithWall(Zone zone, Rect2D rect, string wallObject)
	{
		for (int i = rect.x1; i <= rect.x2; i++)
		{
			...
			{
				zone.GetCell(i, rect.y1).ClearObjectsWithTag("Wall");
			}
			if (zone.GetCell(i, rect.y1) != null)
			{
			...
			{
				zone.GetCell(i, rect.y2).ClearObjectsWithTag("Wall");
			}
			if (zone.GetCell(i, rect.y2) != null)
			{
		...
		for (int j = rect.y1 + 1; j <= rect.y2 - 1; j++)
		{
			...
			{
				zone.GetCell(rect.x1, j).ClearObjectsWithTag("Wall");
			}
			if (zone.GetCell(rect.x1, j) != null)
			{
			...
			{
				zone.GetCell(rect.x2, j).ClearObjectsWithTag("Wall");
			}
			if (zone.GetCell(rect.x2, j) != null)
			{
	...
	public static bool PlaceObjectInRegionRect(Zone Z, ILocationArea Region, Rect2D R, string FrontSide, string Object, string Hint = null, string Owner = null, List<Location2D> AvoidPoints = null, GameObject precreated = null, Func<string, GameObject> customFabricator = null)
	{
		...
		else
		{
			...
			case "AlongWall":
				...
				{
					if (Z.GetCell(item).GetLocalAdjacentCells().Any((Cell c) => c.HasObjectWithTag("Wall")))
					{
			...
			case "Inside":
			{
				for (int k = R.x1 + 1; k <= R.x2 - 1; k++)
				{
					for (int l = R.y1 + 1; l <= R.y2 - 1; l++)
					{
						AddPointWithHint(Points, Location2D.get(k, l), R, Hint, AvoidPoints);
					}
				}
				break;
			}
			default:
			{
				for (int i = R.x1; i <= R.x2; i++)
				{
					for (int j = R.y1; j <= R.y2; j++)
					{
						AddPointWithHint(Points, Location2D.get(i, j), R, Hint, AvoidPoints);
					}
				}
				break;
			}
		...
		Points.RemoveAll((Location2D p) => Z.GetCell(p).HasObjectWithTagOrProperty("SpawnBlocker"));
		if (Points.Count == 0)
		{
			for (int num6 = R.x1; num6 <= R.x2; num6++)
			{
				for (int num7 = R.y1; num7 <= R.y2; num7++)
				{
					AddPointWithHint(Points, Location2D.get(num6, num7), R, Hint, AvoidPoints);
				}
			}
		}
		if (Points.Count == 0)
		...
		for (int num8 = 0; num8 < Points.Count; num8++)
		{
			if (Z.GetCell(Points[num8]).Objects.Count == 0)
			{
				if (precreated != null)
				{
					Z.GetCell(Points[num8]).AddObject(precreated).pPhysics.Owner = Owner;
				}
				else
				{
					Z.GetCell(Points[num8]).AddObject(Object).pPhysics.Owner = Owner;
				}
				return true;
			}
		}
	...
	public static void PlaceStructuredPopulationInRegion(Zone Z, InfluenceMapRegion R, StructuredPopulationResult P, List<Location2D> ValidPoints = null)
	{
		List<Location2D> list = new List<Location2D>();
		if (ValidPoints == null)
		{
	...
	public static GameObject ApplyHintsToObject(GameObject gameObject, string hints, int group = 0)
	{
		...
		foreach (string text in array)
		{
			...
			if (!templateApplicators.ContainsKey(array2[0]))
			{
				...
				{
					Debug.LogError("Unresolved tempalte type: " + array2[0]);
				}
			}
	...
	public static bool PlaceObjectInArea(Zone Z, ILocationArea A, GameObject gameObject, int group = 0, int groupordinal = 0, string Hints = null, string Builder = null, bool bAllowCaching = false)
	{
		...
		int num = 0;
		bool flag = gameObject.HasTagOrProperty("SpawnBlocker");
		string[] array = Hints.Split(',');
		foreach (string text in array)
		{
			if (text == "")
			...
			if (!flag2)
			{
				...
				else if (text.StartsWith("StackWithSelf"))
				{
				...
				else if (text.StartsWith("LivesOnWalls"))
				{
					...
					foreach (Location2D item15 in A.EnumerateLocations())
					{
						...
						{
							if (localAdjacentCell.HasObjectWithTag("Wall") && !localAdjacentCell.HasCombatObject() && !Points.Contains(localAdjacentCell.location))
							{
					...
					if (Points.Count == 0)
					{
						foreach (Cell passableCell in Z.GetPassableCells())
						{
							...
							{
								if (localAdjacentCell2.HasObjectWithTag("Wall") && !localAdjacentCell2.HasCombatObject() && !Points.Contains(localAdjacentCell2.location))
								{
					...
					if (Points.Count == 0)
					{
						foreach (Cell reachableCell in Z.GetReachableCells())
						{
							...
							{
								if (localAdjacentCell3.HasObjectWithTag("Wall") && !localAdjacentCell3.HasCombatObject() && !Points.Contains(localAdjacentCell3.location))
								{
					...
					if (Points.Count == 0)
					{
						foreach (Cell emptyCell in Z.GetEmptyCells())
						{
							...
							{
								if (localAdjacentCell4.HasObjectWithTag("Wall") && !localAdjacentCell4.HasCombatObject() && !Points.Contains(localAdjacentCell4.location))
								{
				...
				else if (text == "InsideCorner")
				{
					...
					if (Points.Count == 0)
					{
						...
						{
							if (Z.GetCell(item17).GetLocalAdjacentCells().Any((Cell c) => c.HasObjectWithTag("Wall")) && !Points.Contains(item17))
							{
								Points.Add(item17);
							}
						}
				...
				else if (text.StartsWith("AlongWall:"))
				{
					...
					foreach (Location2D item18 in A.EnumerateLocations())
					{
						...
						if (cellFromDirection != null && cellFromDirection.HasObjectWithTag("Wall") && !Points.Contains(cell2.location))
						{
				...
				else if (text == "AlongWall")
				{
					...
					{
						if (Z.GetCell(item20).GetLocalAdjacentCells().Any((Cell c) => c.HasObjectWithTag("Wall")) && !Points.Contains(item20))
						{
							Points.Add(item20);
						}
					}
		...
		{
			if (gameObject.pBrain.LivesOnWalls)
			{
				workingSet.RemoveAll((Location2D p) => !Z.GetCell(p).HasWall());
			}
			if (gameObject.pBrain.Aquatic)
			{
		...
		{
			workingSet.RemoveAll((Location2D l) => Z.GetCell(l).HasObjectWithTagOrProperty("SpawnBlocker"));
		}
		...
		if (Options.GetOption("OptionDrawPopulationHintMaps") == "Yes")
		{
			...
			for (int k = 0; k < 80; k++)
			{
				for (int m = 0; m < 25; m++)
				{
					...
					{
						if (Z.GetCell(k, m).HasObjectWithTag("Wall"))
						{
							scrapBuffer.Write("&g#");
						}
						else
					...
					else if (Z.GetCell(k, m).HasObjectWithTag("Wall"))
					{
						scrapBuffer.Write("&K#");
					}
					else
	...
	public static bool PlaceObjectInRect(Zone Z, Rect2D R, Rect2D Exclude, string Object, string Hint = null, int group = 0, Func<string, GameObject> customFabricator = null)
	{
		...
		for (int i = R.x1; i <= R.x2; i++)
		{
			for (int j = R.y1; j <= R.y2; j++)
			{
				if (!Exclude.PointIn(Location2D.get(i, j)))
				{
	...
	public static void PlacePrefab(Zone Z, string mapName, Cell upperLeft, int Padding = 0, string ChunkHint = null, Action<Cell> PreAction = null, Action<Cell> PostAction = null, Func<string, Cell, bool> ShouldPlace = null, Func<string, Cell, string> Replace = null, Action<string, Cell> BeforePlacement = null, Action<string, Cell> AfterPlacement = null)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				Cell cell = Z.GetCell(pos2D.x + i, pos2D.y + j);
				if (cell != null)
				{
					mapFile.Cells[i, j].ApplyTo(cell, CheckEmpty: false, PreAction, PostAction, ShouldPlace, Replace, BeforePlacement, AfterPlacement);
				}
			}
		}
	}
	...
	public static void PlaceParty(List<GameObject> Party, Zone Z, int Radius = -1)
	{
		...
		for (int i = 0; i < 500; i++)
		{
			...
			for (int j = rect2D.x1; j <= rect2D.x2; j++)
			{
				for (int k = rect2D.y1; k <= rect2D.y2; k++)
				{
					Location2D location2D = Location2D.get(j, k);
	...
	public static void PlaceEncounter(Zone Z, string Table, string Amount = "Low", Dictionary<string, string> Parameters = null)
	{
		...
		if (Parameters != null)
		{
			...
			{
				zoneEncounterBlueprint.Parameters.Add(key, Parameters[key]);
			}
		}
		ZoneManager.ApplyEncounterBlueprintToZone(zoneEncounterBlueprint, Z);
	}
	...
	public static bool PlaceObject(Zone Z, InfluenceMapRegion R, GameObject obj, string Hint = null, string Builder = null, bool bAllowCaching = false)
	{
		Points.Clear();
		bool flag = false;
		if (obj.pBrain != null && obj.pBrain.Aquatic)
		{
			Hint = "Aquatic";
		}
		...
		else if (Hint.StartsWith("StackWithSelf"))
		{
		...
		else if (Hint.StartsWith("LivesOnWalls"))
		{
			foreach (Location2D borderCell in R.BorderCells)
			{
				...
				{
					if (localAdjacentCell.HasObjectWithTagOrProperty("Wall") && !Points.Contains(localAdjacentCell.location))
					{
						Points.Add(localAdjacentCell.location);
					}
				}
		...
		else if (Hint == "AlongWall")
		{
			foreach (Location2D cell4 in R.Cells)
			{
				...
				if (!cell.HasObjectWithTagOrProperty("Wall"))
				{
					continue;
				}
				cell.ForeachLocalAdjacentCell(delegate(Cell AC)
				{
					if (!AC.HasObjectWithTagOrProperty("Wall") && !Points.Contains(AC.location))
					{
