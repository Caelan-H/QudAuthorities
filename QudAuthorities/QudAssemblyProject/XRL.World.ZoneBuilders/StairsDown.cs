...
public class StairsDown
{
	...
	[NonSerialized]
	private static string[] UpStairsBlueprints = new string[4] { "StairsUp", "OpenShaft", "Pit", "StairBlocker" };
	...
	public static bool AddStairDown(Zone Z, string x, string y, bool Reachable, bool EmptyOnly)
	{
		...
		if (num == -1 || num3 == -1 || num2 - num != 0 || num4 - num3 != 0)
		{
			...
			{
				if (item2.GetPart("StairsDown") is XRL.World.Parts.StairsDown stairsDown && !item2.HasTagOrProperty("IgnoreForStairConnections") && stairsDown.Connected)
				{
					return true;
				}
			}
