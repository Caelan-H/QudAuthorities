	...
	public bool[,] GetWatermap(int Wx, int Wy)
	{
		...
		for (int k = -1; k <= 1; k++)
		{
			for (int l = -1; l <= 1; l++)
			{
				if (Wy + l >= 0)
				{
					array2[k + 1, l + 1] = zone.GetCell(Wx + k, Wy + l)?.HasObjectWithPropertyOrTagEqualToValue("Terrain", "LakeHinnom") ?? false;
				}
				else
				{
					array2[k + 1, l + 1] = true;
		...
		if (Z._ZoneID.Contains("JoppaWorld"))
		{
			...
			for (int num11 = 0; num11 < Z.Width; num11++)
			{
				for (int num12 = 0; num12 < Z.Height; num12++)
				{
					...
					{
						Z.GetCell(num11, num12).AddObject(GameObjectFactory.Factory.CreateObject("AlgalWaterDeepPool"));
					}
				}
			}
