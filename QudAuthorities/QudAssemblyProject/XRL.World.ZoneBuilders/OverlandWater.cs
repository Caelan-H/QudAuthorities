	...
	public bool[,] GetWatermap(int Wx, int Wy)
	{
		...
		for (int k = -1; k <= 1; k++)
		{
			for (int l = -1; l <= 1; l++)
			{
				if (Wy + l >= 0)
				{
					Cell cell = zone.GetCell(Wx + k, Wy + l);
					array2[k + 1, l + 1] = cell.HasObjectWithPropertyOrTagEqualToValue("Terrain", "Water");
				}
				else
				{
					array2[k + 1, l + 1] = true;
		...
		if (Z._ZoneID.Contains("JoppaWorld"))
		{
			...
			if (!Underground)
			{
				...
				for (int num9 = 0; num9 < Z.Width; num9++)
				{
					...
					{
						if ((double)noiseMap.Noise[num9, num10] > 0.01)
						{
							Z.GetCell(num9, num10).Clear();
						}
					}
				}
			}
			...
			{
				for (int num12 = 0; num12 < Z.Height; num12++)
				{
					if ((double)noiseMap.Noise[num11, num12] > 0.01)
					{
						Z.GetCell(num11, num12).AddObject(GameObjectFactory.Factory.CreateObject("SaltyWaterDeepPool"));
					}
				}
			}
