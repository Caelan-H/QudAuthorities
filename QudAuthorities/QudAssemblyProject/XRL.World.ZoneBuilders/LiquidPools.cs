	...
	public bool BuildZone(Zone Z, string PuddleObject, int Density, string FlamingPools = "0", string PlantReplacements = null)
	{
		...
		for (int j = 0; j < Z.Width; j++)
		{
			for (int k = 0; k < Z.Height; k++)
			{
				...
				{
					if (Z.GetCell(j, k).IsEmpty())
					{
						list2.Add(Z.GetCell(j, k).AddObject(GameObjectFactory.Factory.CreateObject(PuddleObject)));
					}
				}
				else if ((double)noiseMap.Noise[j, k] >= 0.8 && PlantReplacements != null)
				{
					list3.Add(Z.GetCell(j, k));
				}
			}
		}
		...
		if (num4 > 0)
		{
			...
			{
				list2[num5].pPhysics.Temperature = (list2[num5].pPhysics.FlameTemperature + list2[num5].pPhysics.VaporTemperature) / 2;
			}
		}
		return true;
	}
