...
public class JoppaOutskirts : ZoneBuilderSandbox
{
	public bool BuildZone(Zone Z)
	{
		...
		for (int num9 = 0; num9 < Z.Width; num9++)
		{
			...
			{
				double num11 = (double)Watervine.WatervineNoise[num9 + num, num10 + num2] + array[num9, num10];
				if (noiseMap.Noise[num9, num10] >= 5)
				{
					Z.GetCell(num9, num10).AddObject(GameObjectFactory.Factory.CreateObject("SaltyWaterPuddle"));
				}
				else if ((double)noiseMap.Noise[num9, num10] >= 3.5)
				{
					Z.GetCell(num9, num10).AddObject(GameObjectFactory.Factory.CreateObject("Watervine"));
				}
				else if (num11 >= 0.8)
				{
					Z.GetCell(num9, num10).AddObject(GameObjectFactory.Factory.CreateObject("SaltyWaterPuddle"));
				...
				{
					Z.GetCell(num9, num10).SetReachable(State: true);
				}
			}
		}
		...
		if (Z.X == 1 && Z.Y == 0)
		{
			...
			if (influenceMap.Regions.Count > 0)
			{
				...
				ClearRect(Z, r2);
				ZoneBuilderSandbox.PlaceObjectOnRect(Z, "WoodWall", r2);
				...
				List<Cell> cells = Z.GetCells(delegate(Cell c)
				{
					...
					{
						for (int num20 = 0; num20 < prefabHeight; num20++)
						{
							Cell cellFromOffset = c.GetCellFromOffset(num19, num20);
				...
				for (int num15 = 0; num15 < prefabWidth; num15++)
				{
					for (int num16 = 0; num16 < prefabHeight; num16++)
					{
						randomElement2.GetCellFromOffset(num15, num16).Clear();
					}
				}
		...
		{
			for (int num18 = 0; num18 < Z.Height; num18++)
			{
				if (Z.GetCell(num17, num18).HasObjectWithTag("Wall"))
				{
					pathfinder.CurrentNavigationMap[num17, num18] = 9999;
				}
				else if (Z.GetCell(num17, num18).HasObjectWithPart("LiquidVolume"))
				{
					pathfinder.CurrentNavigationMap[num17, num18] = 200;
				}
				else
				{
					pathfinder.CurrentNavigationMap[num17, num18] = Stat.Random(0, 50);
				}
			}
		}
		...
		{
			if (!pathfinder.FindPath(cell.location, item.location, bDisplay: false, bOrdinalDirectionsOnly: true))
			{
				continue;
			}
