...
public class PigFarm
{
	...
	public bool BuildZone(Zone Z, bool ClearCombatObjectsFirst, string WallObject, string ZoneTable, string Widgets)
	{
		...
		while (true)
		{
			if (num > 0)
			{
				list = Tools.GenerateBoxes(BoxGenerateOverlap.Irrelevant, new Range(1, 3), new Range(40, 60), new Range(8, 16), null);
				if (list == null)
				{
					return false;
				}
		...
		foreach (Box item2 in list)
		{
			...
			for (int n = item2.y1 + 1; n < item2.y2; n++)
			{
		...
		for (int num9 = 0; num9 < 80; num9++)
		{
			...
			{
				if (Z.GetCell(num9, num10).HasObjectWithBlueprint("BrinestalkFence"))
				{
					list3.Add(Z.GetCell(num9, num10));
				}
			}
		}
		if (list3.Count > 0)
		{
			for (int num11 = 0; num11 < Stat.Random(1, 3); num11++)
			{
		...
		int num12 = "8d2".RollCached();
		num = 200;
		while (num12 > 0 && num > 0 && list2.Count > 0)
		{
