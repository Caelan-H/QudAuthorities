	...
	public bool BuildZone(Zone Z, bool ClearCombatObjectsFirst, string WallObject, string ZoneTable, string Widgets, string CustomRoomPopulation = null, string BoxWidth = "30-50", string BoxHeight = "16-24", bool SpecialRedrockBuilder = false)
	{
		...
		if (ClearCombatObjectsFirst)
		{
			for (int k = box.x1; k <= box.x2; k++)
			{
				for (int l = box.y1; l <= box.y2; l++)
				{
					while (true)
					{
						using (List<GameObject>.Enumerator enumerator = Z.GetCell(k, l).GetObjectsWithPart("Combat").GetEnumerator())
						{
							if (enumerator.MoveNext())
							{
								GameObject current = enumerator.Current;
								Z.GetCell(k, l).RemoveObject(current);
								continue;
							}
						}
						break;
		...
		foreach (Box item2 in list2)
		{
			...
			else if (item2.Volume > 25 && item2.Height > 6 && item2.Width > 4)
			{
				...
				for (int num3 = 1; num3 < buildingTemplate.Width - 1; num3++)
				{
					for (int num4 = 1; num4 < buildingTemplate.Height - 1; num4++)
					{
						...
						{
							Z.GetCell(num3 + item2.x1, num4 + item2.y1).ClearObjectsWithIntProperty("Wall");
						}
					}
				}
			...
			while (num < 1000)
			{
				...
				if (num6 != box.x1 && num6 != box.x2 && num7 != box.y1 && num7 != box.y2 && ((!Z.GetCell(num6 - 1, num7).HasObjectWithIntProperty("Wall") && !Z.GetCell(num6 + 1, num7).HasObjectWithIntProperty("Wall")) || (!Z.GetCell(num6, num7 - 1).HasObjectWithIntProperty("Wall") && !Z.GetCell(num6, num7 + 1).HasObjectWithIntProperty("Wall"))))
				{
		...
		for (int num10 = 0; num10 < 2; num10++)
		{
			...
			if (((uint)num11 & (true ? 1u : 0u)) != 0)
			{
				...
				if (!Z.GetCell(num9, box.y1 + 2).HasObjectWithIntProperty("Wall") && Z.GetCell(num9, box.y1).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9, box.y1).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9, box.y1).AddObject("BrinestalkArrowslit");
					Z.GetCell(num9, box.y1 + 1).ClearObjectsWithIntProperty("Wall");
			...
			{
				num9 = Stat.Random(box.x1 + 3, box.x2 - 3);
				if (!Z.GetCell(num9, box.y2 - 2).HasObjectWithIntProperty("Wall") && Z.GetCell(num9, box.y2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9, box.y2).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9, box.y2 - 1).ClearObjectsWithIntProperty("Wall");
			...
			if (((uint)num11 & 4u) != 0)
			{
				...
				if (!Z.GetCell(box.x1 + 2, y2).HasObjectWithIntProperty("Wall") && Z.GetCell(box.x1, y2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x1, y2).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x1 + 1, y2).ClearObjectsWithIntProperty("Wall");
			...
			if (((uint)num11 & 8u) != 0)
			{
				...
				if (!Z.GetCell(box.x2 - 2, y3).HasObjectWithIntProperty("Wall") && Z.GetCell(box.x2, y3).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x2, y3).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x2 - 1, y3).ClearObjectsWithIntProperty("Wall");
		...
		do
		{
			...
			if (((uint)num13 & (true ? 1u : 0u)) != 0 || num14 != -1)
			{
				num9 = Stat.Random(box.x1 + 3, box.x2 - 3);
				if (num14 != -1 && num12 < 10)
				{
				...
				if (!Z.GetCell(num9, box.y1 + 2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9, box.y1).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9, box.y1 + 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
				if (!Z.GetCell(num9 + 1, box.y1 + 2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9 + 1, box.y1).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9 + 1, box.y1 + 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
			}
			if (((uint)num13 & 2u) != 0 || num15 != -1)
			{
				num9 = Stat.Random(box.x1 + 3, box.x2 - 3);
				if (num15 != -1 && num12 < 10)
				{
				...
				if (!Z.GetCell(num9, box.y2 - 2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9, box.y2).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9, box.y2 - 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
				if (!Z.GetCell(num9 + 1, box.y2 - 2).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(num9 + 1, box.y2).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(num9 + 1, box.y2 - 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
			}
			if (((uint)num13 & 4u) != 0 || num16 != -1)
			{
				...
				if (!Z.GetCell(box.x1 + 2, num20).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x1, num20).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x1 + 1, num20).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
				if (!Z.GetCell(box.x1 + 2, num20 + 1).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x1, num20 + 1).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x1 + 1, num20 + 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
			}
			if (((uint)num13 & 8u) != 0 || num17 != -1)
			{
				...
				if (!Z.GetCell(box.x2 - 2, num21).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x2, num21).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x2 - 1, num21).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
				if (!Z.GetCell(box.x2 - 2, num21 + 1).HasObjectWithIntProperty("Wall"))
				{
					Z.GetCell(box.x2, num21 + 1).ClearObjectsWithIntProperty("Wall");
					Z.GetCell(box.x2 - 1, num21 + 1).ClearObjectsWithIntProperty("Wall");
					flag2 = true;
				}
			}
		}
