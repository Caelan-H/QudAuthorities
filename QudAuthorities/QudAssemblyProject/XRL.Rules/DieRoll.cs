...
public class DieRoll
{
	...
	private void IncorporateLeft(string Dice)
	{
		if (!int.TryParse(Dice, out LeftValue))
		{
			Left = new DieRoll(Dice);
			if (Left.Type == 3)
			{
	...
	private void IncorporateRight(string Dice)
	{
		if (!int.TryParse(Dice, out RightValue))
		{
			Right = new DieRoll(Dice);
			if (Right.Type == 3)
			{
	...
	public void Parse(string Dice)
	{
		...
		if (Dice[0] == '-')
		{
			...
			Type = 8;
			IncorporateLeft(Dice.Substring(1));
		}
		else if (Dice.Contains("|"))
		{
			...
			case 2:
				Type = 9;
				IncorporateLeft(array[0]);
				IncorporateRight(array[1]);
				return;
			}
			Type = 9;
