	...
	public static int RollDamagePenetrations(int TargetInclusive, int Bonus, int MaxBonus)
	{
		...
		while (num2 == 3)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				num4 += num3;
				int num5 = num4 + ((Bonus > MaxBonus) ? MaxBonus : Bonus);
			...
			if (debugDamagePenetrations)
			{
				MessageQueue.AddPlayerMessage("{{K|Penning Bonus: " + Bonus + " Max: " + MaxBonus + " Used: " + ((Bonus > MaxBonus) ? MaxBonus : Bonus) + " Target: " + TargetInclusive + "(Penned " + num2 + " times)}}");
			}
			if (num2 >= 1)
			{
				num++;
