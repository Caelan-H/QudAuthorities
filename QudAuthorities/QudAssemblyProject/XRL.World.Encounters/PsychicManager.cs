...
[Serializable]
public class PsychicManager : IGamestatePostload, IGamestatePostsave
{
	...
	public void OnGamestatePostsave(XRLGame game, SerializationWriter writer)
	{
		writer.Write(PsychicFactions.Count);
		foreach (PsychicFaction psychicFaction in PsychicFactions)
		{
			writer.Write(psychicFaction.dimensionalTraining);
		}
		writer.Write(ExtraDimensions.Count);
		foreach (ExtraDimension extraDimension in ExtraDimensions)
		{
			writer.Write(extraDimension.Training);
		}
	}

	public static void Init()
	{
		PsychicManager psychicManager = new PsychicManager();
		for (int i = 1; i < 4; i++)
		{
			psychicManager.PsychicFactions.Add(psychicManager.InitializeFaction());
		}
		psychicManager.GenerateMoreDimensions();
		XRLCore.Core.Game.SetObjectGameState("PsychicManager", psychicManager);
	}
	...
	public PsychicFaction InitializeFaction()
	{
		...
		if (80.in100())
		{
			List<MutationEntry> list = new List<MutationEntry>(MutationFactory.AllMutationEntries());
			list.ShuffleInPlace();
	...
	public void GenerateMoreDimensions()
	{
		...
		JournalAPI.AddObservation("There exists a pocket dimension known as {{O|Tzimtzlum}}.", "$Tzimtzlum", "Dimensions", "$Tzimtzlum", new string[1] { "dimension" }, revealed: false, -1L, "There exists a pocket dimension known as {{O|Tzimtzlum}}.\n\n");
	}
