...
public class QudMutationsModule : EmbarkBuilderModule<QudMutationsModuleData>
{
	...
	public override SummaryBlockData GetSummaryBlock()
	{
		...
		if (base.data != null)
		{
			foreach (QudMutationModuleDataRow selection in base.data.selections)
			{
				...
				{
					stringBuilder.Append(selection.Mutation);
					if (selection.entry.IsDefect())
					{
					...
					{
						stringBuilder.AppendFormat(" ({0})", selection.variantName);
					}
