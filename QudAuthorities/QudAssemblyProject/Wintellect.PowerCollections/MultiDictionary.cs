...
public class MultiDictionary<TKey, TValue> : MultiDictionaryBase<TKey, TValue>, ICloneable
{
	...
	{
		KeyAndValues itemDeleted = new KeyAndValues(key);
		if (hash.Find(itemDeleted, replace: false, out var item))
		{
			...
			if (count == 1)
			{
				hash.Delete(item, out itemDeleted);
				return true;
			}
			if (num >= 0)
			{
				if (num < count - 1)
				{
					Array.Copy(item.Values, num + 1, item.Values, num, count - num - 1);
				}
				item.Count = count - 1;
				hash.Find(item, replace: true, out itemDeleted);
				return true;
			}
			return false;
		}
