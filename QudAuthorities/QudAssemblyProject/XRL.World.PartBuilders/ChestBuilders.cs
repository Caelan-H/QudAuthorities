...
public class ChestBuilders
{
	public static void BuildCommonChestInventory(Inventory pInventory, int tier, string Context = null)
	{
		...
		for (int i = 0; i < num; i++)
		{
			try
			{
				pInventory.AddObject(PopulationManager.CreateOneFrom("Junk " + Tier.Fuzz(tier), null, 0, 0, Context));
			}
			catch (Exception x)
			{
				if (!flag)
	...
	public static void BuildTableChestInventory(Inventory pInventory, string Table, string Amount, int tier, string Context = null)
	{
		...
		for (int i = 0; i < num; i++)
		{
			try
			{
				pInventory.AddObject(PopulationManager.CreateOneFrom(Table + Tier.Fuzz(tier), null, 0, 0, Context));
			}
			catch (Exception x)
			{
				if (!flag)
	...
	public static void BuildRareChestInventory(Inventory pInventory, int tier, string Context = null)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				Tier.Fuzz(tier);
				pInventory.AddObject(PopulationManager.CreateOneFrom("Junk " + tier + "R", null, 17, 0, Context));
			}
			catch (Exception x)
			{
				if (!flag)
	...
	public static void BuildSpecialChestInventory(Inventory pInventory, int Tier, string Context = null)
	{
		...
		for (int num = dice.RollCached(); i < num; i++)
		{
			try
			{
				pInventory.AddObject(PopulationManager.CreateOneFrom(populationName, null, 0, 0, Context));
			}
			catch (Exception x)
			{
				if (!flag)
