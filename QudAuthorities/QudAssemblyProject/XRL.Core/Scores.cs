...
public class Scores : IWantsTextConsoleInit
{
	...
	public static void Show()
	{
		...
		do
		{
			...
			Buffer.Goto(0, 0);
			keys = Keys.None;
			if (GameManager.Instance.OverlayUIEnabled)
			{
				if (num == 0)
				{
					array = array2;
				...
				if (array != null)
				{
					List<string> resultsToPost = new List<string>(array);
					int playerpos = playersPosition;
					string boardtitle = text;
					GameManager.Instance.uiQueue.queueTask(delegate
					{
						HighScoresView.instance.SetLeaderboardResults(boardtitle, resultsToPost, playerpos);
					});
				}
			}
			Buffer.Goto(2, 0);
			...
			if (num == 0)
			{
				...
				else
				{
					...
					if (((num == 0 && keys == Keys.D) || keys == Keys.Delete) && num2 >= 0 && num2 < Scoreboard.Scores.Count && Popup.ShowYesNo("Are you sure you want to delete this?\n\n" + dictionary[Scoreboard.Scores[num2]][0]) == DialogResult.Yes)
					{
			...
			if (keys == Keys.MouseEvent && Keyboard.CurrentMouseEvent.Event.StartsWith("Page:"))
			{
				GameManager.Instance.uiQueue.queueTask(delegate
				{
					HighScoresView.instance.SetLeaderboardResults("Loading...", new List<string>(), -1);
				});
			...
			if (flag && SteamManager.Initialized)
			{
				string boardID = null;
				if (num == 1)
				{
