...
public class ItemNamingSifrah : RitualSifrah
{
	...
	public ItemNamingSifrah(GameObject ContextObject, int Rating, int Difficulty)
	{
		...
		int num = Math.Max(Difficulty * 4 / 3, 3);
		int num2 = 4;
		if (Difficulty >= 5)
		{
			num2++;
		...
		if (num3 > 0)
		{
			...
			if (Rating >= 7 && !The.Player.HasEffect("Lost") && The.Player.CanApplyEffect("Lost"))
			{
				int num10 = Math.Max(25 + (Difficulty - Rating) * 10, 5);
				if (num10 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectLost(num10));
				}
			}
			if (Rating >= 2 && !The.Player.HasEffect("Bleeding") && The.Player.CanApplyEffect("Bleeding"))
			{
				int num11 = Math.Max(20 + (Difficulty - Rating) * 5, 5);
				if (num11 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectBleeding(num11));
				}
			}
			if (Rating >= 2 && !The.Player.HasEffect("Poisoned") && The.Player.CanApplyEffect("Poisoned", 0, "CanApplyPoison"))
			{
				int num12 = Math.Max(10 + (Difficulty - Rating) / 2 * 5, 5);
				if (num12 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectPoisoned(num12));
				}
			}
			if (Rating >= 3 && !The.Player.HasEffect("Ill") && !The.Player.HasEffect("Poisoned") && The.Player.CanApplyEffect("Ill"))
			{
				int num13 = Math.Max(30 + (Difficulty - Rating) * 5, 5);
				if (num13 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectIll(num13));
				}
			}
			if (Rating >= 5 && !The.Player.HasEffect("CardiacArrest") && The.Player.CanApplyEffect("CardiacArrest"))
			{
				...
				if (num14 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectCardiacArrest(num14));
				}
			}
			if (Rating >= 12 && !The.Player.HasEffect("ShatterMentalArmor") && The.Player.CanApplyEffect("ShatterMentalArmor"))
			{
				int num15 = Math.Max(40 + (Difficulty - Rating) * 5, 5);
				if (num15 <= 120)
				{
					list2.Add(new RitualSifrahTokenEffectShatterMentalArmor(num15));
				}
			}
		...
		{
			Popup.ShowFail("You have no usable options to employ for ritually naming " + ContextObject.t() + ", giving you no chance of performing well. You can remedy this situation by improving your Ego, Willpower, and esoteric skills, or by obtaining items useful in ritual.");
			Finished = true;
			Abort = true;
		}
	}
	...
	public override void CalculateOutcomeChances(out int Success, out int Failure, out int PartialSuccess, out int CriticalSuccess, out int CriticalFailure)
	{
		...
		double num3 = 0.03 + (double)Powerup * 0.01;
		if (Turn > 1)
		{
