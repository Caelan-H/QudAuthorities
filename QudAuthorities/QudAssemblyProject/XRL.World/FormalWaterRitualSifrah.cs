...
public class FormalWaterRitualSifrah : RitualSifrah
{
	...
	public FormalWaterRitualSifrah(GameObject ContextObject)
	{
		...
		int num = Math.Max(Difficulty * 5 / 4, 3);
		int num2 = 4;
		if (Difficulty >= 4)
		{
			num2++;
		...
		{
			Popup.ShowFail("You have no usable options to employ for performing the formal water ritual with " + ContextObject.t() + ", giving you no chance of doing so well. You can remedy this situation by improving your Ego or by obtaining items useful in such a ritual.");
			Finished = true;
			Abort = true;
		}
	}
