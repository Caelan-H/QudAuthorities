...
public class VillageDynamicQuestContext : DynamicQuestContext
{
	...
	public List<Location2D> usedLocations = new List<Location2D>();

	public WorldInfo worldInfo => XRLCore.Core.Game.GetObjectGameState("JoppaWorldInfo") as WorldInfo;
	...
	public override DynamicQuestReward getQuestReward()
	{
		if (isVillageZero)
		{
			if (questNumber == 0)
			{
				...
				gameObject.pRender.DisplayName = villageSnapshot.GetProperty("name") + " recoiler";
	...
	public override GameObject getQuestDeliveryItem()
	{
		...
		case 1:
			...
			break;
		default:
			gameObject2 = EncountersAPI.GetAnItem((GameObjectBlueprint ob) => ob.Tier <= villageSnapshot.Tier && ob.GetPartParameter("Physics", "Takeable", "true") == "true" && int.Parse(ob.GetPartParameter("Physics", "Weight", "0")) <= 100 && !ob.HasTag("ExcludeFromQuests"));
		...
		gameObject2.pRender.SetForegroundColor(villageSnapshot.GetList("palette")[0]);
		gameObject2.pRender.DetailColor = villageSnapshot.GetList("palette")[1];
