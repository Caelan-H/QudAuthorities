...
public class ItemModdingSifrah : TinkeringSifrah
{
	...
	{
		Description = "Modding " + ContextObject.an(int.MaxValue, null, null, AsIfKnown: false, Single: true, NoConfusion: false, NoColor: false, Stripped: false, WithoutEpithet: false, Short: false);
		...
		int @for = GetTinkeringBonusEvent.GetFor(The.Player, ContextObject, "ItemModding", MaxTurns, 0, ref Interrupt);
		if (Interrupt)
		{
			Finished = true;
			Abort = true;
		...
		{
			Popup.ShowFail("You have no usable options to employ for modding " + ContextObject.t() + ", giving you no chance of success. You can remedy this situation by improving your Intelligence and tinkering skills, or by obtaining items useful for tinkering.");
			Finished = true;
			Abort = true;
		}
	}
	...
	{
		Popup.Show("You abjectly failed to mod " + ContextObject.t() + ".");
		Performance = 0;
	}

	private void ResultPartialSuccess(GameObject ContextObject)
	{
		Popup.Show("Your work modding " + ContextObject.t() + " was passable.");
		Performance = 0;
		ApplyMod = true;
	}
	...
	{
		Popup.Show("Your work modding " + ContextObject.t() + " was solid and craftsmanlike.");
		Performance = 1;
		ApplyMod = true;
	}
	...
	{
		Popup.Show("Your work modding " + ContextObject.t() + " was outstanding.");
		Performance = Stat.Random(2, 3);
		ApplyMod = true;
	}
