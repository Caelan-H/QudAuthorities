...
public static class Anatomies
{
	...
	private static void Init()
	{
		...
		ProcessBodiesXmlFile("Bodies.xml", mod: false);
		ModManager.ForEachFile("Bodies.xml", delegate(string file)
		{
			ProcessBodiesXmlFile(file, mod: true);
		});
	}
	...
	private static void LoadBodyPartInfo(BodyPartType Entry, XmlTextReader Reader, bool mod = false)
	{
		...
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.DescriptionPrefix = attribute;
		}
		...
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.LimbBlueprintProperty = attribute;
		}
		attribute = Reader.GetAttribute("LimbBlueprintDefault");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.LimbBlueprintDefault = attribute;
		}
		attribute = Reader.GetAttribute("Category");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Category = BodyPartCategory.GetCode(attribute);
		}
		attribute = Reader.GetAttribute("Laterality");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Laterality = Laterality.GetCode(attribute);
		}
		attribute = Reader.GetAttribute("ImpliedPer");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.ImpliedPer = TryInt(attribute, "ImpliedPer attribute");
		}
		attribute = Reader.GetAttribute("RequiresLaterality");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.RequiresLaterality = Laterality.GetCode(attribute);
		}
		attribute = Reader.GetAttribute("Mobility");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Mobility = TryInt(attribute, "Mobility attribute");
		}
		attribute = Reader.GetAttribute("ChimeraWeight");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.ChimeraWeight = TryInt(attribute, "ChimeraWeight attribute");
		}
		attribute = Reader.GetAttribute("Appendage");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Appendage = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Integral");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Integral = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Abstract");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Abstract = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Mortal");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Mortal = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Extrinsic");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Extrinsic = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Plural");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Plural = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Mass");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Mass = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("Contact");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.Contact = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("IgnorePosition");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.IgnorePosition = Convert.ToBoolean(attribute);
		}
		attribute = Reader.GetAttribute("NoArmorAveraging");
		if (!string.IsNullOrEmpty(attribute))
		{
			Entry.NoArmorAveraging = Convert.ToBoolean(attribute);
		}
