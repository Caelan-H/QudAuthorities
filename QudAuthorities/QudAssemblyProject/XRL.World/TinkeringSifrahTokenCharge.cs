...
public class TinkeringSifrahTokenCharge : SifrahPrioritizableToken
{
	...
	{
		if (The.Player.GetPart("ElectricalGeneration") is ElectricalGeneration electricalGeneration && electricalGeneration.GetCharge() >= Amount)
		{
			return int.MaxValue;
		}
		int num = 0;
	...
	public override bool CheckTokenUse(SifrahGame Game, SifrahSlot Slot, GameObject ContextObject)
	{
		if (!IsAvailable())
		{
			if (The.Player.GetPart("ElectricalGeneration") is ElectricalGeneration)
			{
	...
	public override void UseToken(SifrahGame Game, SifrahSlot Slot, GameObject ContextObject)
	{
		...
		if (The.Player.GetPart("ElectricalGeneration") is ElectricalGeneration electricalGeneration && electricalGeneration.GetCharge() >= Amount)
		{
			electricalGeneration.UseCharge(Amount);
			flag = true;
		}
		if (!flag)
		{
