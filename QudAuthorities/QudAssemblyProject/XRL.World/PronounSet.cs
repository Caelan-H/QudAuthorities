...
public class PronounSet : BasePronounProvider
{
	...
	public static void Init()
	{
		...
		Action<string, bool> ProcessPronounSetXmlFile = delegate(string file, bool mod)
		{
		...
		ProcessPronounSetXmlFile("PronounSets.xml", arg2: false);
		ModManager.ForEachFile("PronounSets.xml", delegate(string file)
		{
			ProcessPronounSetXmlFile(file, arg2: true);
		});
		if (ReplicateGenders)
		{
			foreach (Gender item in Gender.GetAll())
			{
				if (!item.DoNotReplicateAsPronounSet)
				{
					PronounSet pronounSet = new PronounSet(item);
	...
	public static PronounSet Generate(bool Generic = false, int PluralChance = 0, int UseBareIndicativeChance = 0, GameObject For = null, string Genotype = null, string Subtype = null, string Species = null, string Culture = null, string Faction = null)
	{
		...
		NameMaker.MakeName(ref pronounSet._PersonTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderPersonTerm");
		NameMaker.MakeName(ref pronounSet._ImmaturePersonTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderImmaturePersonTerm");
		NameMaker.MakeName(ref pronounSet._FormalAddressTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderFormalAddressTerm");
		NameMaker.MakeName(ref pronounSet._OffspringTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderOffspringTerm");
		NameMaker.MakeName(ref pronounSet._SiblingTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderSiblingTerm");
		NameMaker.MakeName(ref pronounSet._ParentTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderParentTerm");
		return pronounSet;
	}
