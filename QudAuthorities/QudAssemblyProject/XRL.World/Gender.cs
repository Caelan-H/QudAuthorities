...
public class Gender : BasePronounProvider
{
	...
	public override string CapitalizedName
	{
		get
		{
			...
			{
				_CapitalizedName = ConsoleLib.Console.ColorUtility.CapitalizeExceptFormatting(Name);
			}
	...
	private static string GenerateBasePronoun(string stem, string stemVowel, string mainVowel, Dictionary<string, int> suffixes)
	{
		...
		if (!string.IsNullOrEmpty(randomElement) && !text.EndsWith(randomElement))
		{
			text += randomElement;
		}
		if (stem.Length == 1)
		{
			...
			if (!string.IsNullOrEmpty(randomElement2) && randomElement2 != stem)
			{
	...
	public static Gender Generate(bool Generic = false, int PluralChance = 0, int UseBareIndicativeChance = 0, GameObject For = null, string Genotype = null, string Subtype = null, string Species = null, string Culture = null, string Faction = null)
	{
		...
		do
		{
			...
			text = NameMaker.MakeName(For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderName");
		}
		...
		NameMaker.MakeName(ref gender._PersonTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderPersonTerm");
		NameMaker.MakeName(ref gender._ImmaturePersonTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderImmaturePersonTerm");
		NameMaker.MakeName(ref gender._FormalAddressTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderFormalAddressTerm");
		NameMaker.MakeName(ref gender._OffspringTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderOffspringTerm");
		NameMaker.MakeName(ref gender._SiblingTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderSiblingTerm");
		NameMaker.MakeName(ref gender._ParentTerm, For, Genotype, Subtype, Species, Culture, Faction, null, null, null, "GenderParentTerm");
		return gender;
	}
	...
	public static void Init()
	{
		...
		Action<string, bool> ProcessGenderXmlFile = delegate(string file, bool mod)
		{
		...
		ProcessGenderXmlFile("Genders.xml", arg2: false);
		ModManager.ForEachFile("Genders.xml", delegate(string file)
		{
			ProcessGenderXmlFile(file, arg2: true);
		});
		if (string.IsNullOrEmpty(GenerateGenericSpec) || !EnableGeneration)
		{
			return;
		}
	...
	public static void LoadGendersNode(XmlTextReader Reader, ref string GenerateGenericSpec, bool mod = false)
	{
		...
		string attribute3 = Reader.GetAttribute("GenerateGeneric");
		if (!string.IsNullOrEmpty(attribute3))
		{
			GenerateGenericSpec = attribute3;
		}
		while (Reader.Read())
		{
			...
			else if (Reader.Name == "removegender")
			{
				...
				if (Genders.ContainsKey(attribute4))
				{
					Genders.Remove(attribute4);
				}
			}
