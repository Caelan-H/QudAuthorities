...
public class FactionInterest
{
	...
	public string GenerateDescription(Faction faction)
	{
		if (TagList.Count == 1)
		{
			...
			case "deathlands":
				return "locations in the deathlands";
			case "girsh":
			...
			case "rebekah":
				return "Resheph's mentor Rebekah";
			...
			{
				if (value.GetTag("LairAdjectives") == Tags)
				{
					string text2 = Grammar.MakePossessive(Grammar.Pluralize(ColorUtility.StripFormatting(value.DisplayName())));
					string text3 = Grammar.Pluralize(value.GetTag("LairName", "lair"));
