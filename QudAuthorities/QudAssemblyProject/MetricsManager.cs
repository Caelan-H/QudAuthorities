...
public class MetricsManager
{
	...
	public static void LogModError(ModInfo mod, object Message)
	{
		...
		if (Options.ShowErrorPopups)
		{
			...
			Popup.ShowBlock(message, prompt);
		}
	...
	public static void LogError(string Message)
	{
		...
		if (Options.ShowErrorPopups)
		{
			Popup.ShowBlock(Message + "\n" + new StackTrace().ToString(), "{{R|Error}}");
		}
		if (Globals.EnableMetrics)
		{
	...
	private static void _SendTelemetry(string name, object payload, string category = "unassigned", ModInfo modInfo = null, bool jsonPayload = false)
	{
		try
		{
			...
			if (globalTags == null)
			{
				...
				globalTags.Add("version", Assembly.GetExecutingAssembly().GetName().Version.ToString());
			}
			...
			{
				modList.AppendFormat("{0}:{1};", mod.ID, mod.Manifest.Version ?? "unknown version");
			});
			if (modList.Length > 0)
			{
