...
{
	public const string ABL_CMD = "CommandLeyShift";
	...
	{
		if (!base.WantEvent(ID, cascade) && (!Active || ID != BeforeRenderEvent.ID))
		{
	...
	public override bool HandleEvent(CommandEvent E)
	{
		if (E.Command == "CommandLeyShift")
		{
			...
			int renderLayer = ParentObject.pRender.RenderLayer;
			ParentObject.pRender.RenderLayer = 100;
			Active = true;
			...
			{
				Shift(Direction);
				SoundManager.PlaySound((i == Amount) ? "grenade_heat" : "meleeTier8", 0f, 1f, (i == Amount) ? 0.65f : 0.8f);
				if (useOverlayCombatEffects)
				{
	...
	{
		return string.Concat("" + "You shift local space.\n\n", "Cooldown: ", 250.ToString());
	}
	...
	{
		if (E.ID == "AIGetOffensiveMutationList" && E.GetIntParameter("Distance") <= 2 && IsMyActivatedAbilityAIUsable(ActivatedAbilityID))
		{
			E.AddAICommand("CommandLeyShift");
		}
	...
	public void TakeObjects(Cell C)
	{
		for (int num = C.Objects.Count - 1; num >= 0; num--)
		{
			...
			if (gameObject != ParentObject && (gameObject.IsReal || (gameObject.pRender != null && gameObject.pRender.Visible)))
			{
				ShiftList.Add(C.Objects[num]);
				C.RemoveObject(C.Objects[num], Forced: false, System: false, IgnoreGravity: false, NoStack: false, Repaint: false);
			}
		}
	}
	...
	public void ApplyObjects(Cell C)
	{
		...
		{
			C.AddObject(ShiftList2[num], Forced: false, System: false, IgnoreGravity: false, NoStack: false, Repaint: false);
		}
	...
	{
		ActivatedAbilityID = AddMyActivatedAbility("Ley Shift", "CommandLeyShift", "Mental Mutation", null, "\u00a8");
