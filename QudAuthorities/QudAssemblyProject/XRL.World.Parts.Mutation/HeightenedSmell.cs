		...
		if (E.Actor == ParentObject && ParentObject.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				for (int count = list.Count; i < count; i++)
				{
					...
					if (gameObject.GetEffect("HeightenedSmellEffect", IsOurs) is HeightenedSmellEffect heightenedSmellEffect && heightenedSmellEffect.Smeller == ParentObject && heightenedSmellEffect.Identified && ParentObject.IsRelevantHostile(gameObject))
					{
	...
	public override bool FireEvent(Event E)
	{
		if (E.ID == "EndTurn" && ParentObject.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				{
					GameObject gameObject = list[i];
					if (gameObject.IsSmellable(ParentObject) && !gameObject.HasEffect("HeightenedSmellEffect", IsOurs))
					{
