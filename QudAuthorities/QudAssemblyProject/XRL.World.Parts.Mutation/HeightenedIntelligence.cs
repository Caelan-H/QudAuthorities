...
public class HeightenedIntelligence : BaseMutation
{
	...
	{
		string text = "+" + (2 + (Level - 1) / 2) + " Intelligence\n";
	...
	{
		if (E.ID == "BeginTakeAction" && ParentObject.IsPlayer() && ParentObject.pPhysics != null && ParentObject.pPhysics.CurrentCell != null && !ParentObject.pPhysics.CurrentCell.ParentZone.IsWorldMap() && Stat.Random(1, 10000) < (2 + base.Level) * ParentObject.Statistics["Intelligence"].Modifier)
		{
			MessageQueue.AddPlayerMessage("&CA flash of insight overcomes you!");
			ParentObject.pPhysics.CurrentCell.ParentZone.ExploreAll();
		}
