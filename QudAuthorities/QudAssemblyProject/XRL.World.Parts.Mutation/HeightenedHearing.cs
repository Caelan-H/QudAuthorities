		...
		if (E.Actor == ParentObject && ParentObject.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				for (int count = list.Count; i < count; i++)
				{
					...
					if (gameObject.GetEffect("HeightenedHearingEffect", IsOurs) is HeightenedHearingEffect heightenedHearingEffect && heightenedHearingEffect.Listener == ParentObject && heightenedHearingEffect.Identified && ParentObject.IsRelevantHostile(gameObject))
					{
	...
	public override bool FireEvent(Event E)
	{
		if (E.ID == "EndTurn" && ParentObject.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				{
					GameObject gameObject = list[i];
					if (ParentObject.DistanceTo(gameObject) <= radius && !gameObject.HasEffect("HeightenedHearingEffect", IsOurs))
					{
