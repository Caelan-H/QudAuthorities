...
public class SensePsychic : BaseMutation
{
	...
	public override bool WantEvent(int ID, int cascade)
	{
		...
		{
			return ID == ExtraHostilePerceptionEvent.ID;
		}
		return true;
	}
		...
		if (E.Actor == ParentObject && ParentObject.IsPlayer())
		{
			...
			if (cell != null)
			{
				...
				for (int count = list.Count; i < count; i++)
				{
					...
					if (gameObject.GetEffect("SensePsychicEffect", IsOurs) is SensePsychicEffect sensePsychicEffect && sensePsychicEffect.Listener == ParentObject && sensePsychicEffect.Identified && ParentObject.IsRelevantHostile(gameObject))
					{
	...
	{
		return GO.GetEffect("SensePsychicEffect", SensePsychicListenerIsPlayer) as SensePsychicEffect;
	}
	...
	public bool IsOtherReachablePsychic(GameObject obj)
	{
		...
		return obj.LocalEvent("CheckRealityDistortionAccessibility");
	}
	...
	public void SensePsychics()
	{
		...
		List<GameObject> list = ((!RealityDistortionBased) ? cell.ParentZone.FastSquareSearch(cell.X, cell.Y, Radius, IsOtherPsychic) : ((!ParentObject.FireEvent("CheckRealityDistortionUsability")) ? null : cell.ParentZone.FastSquareSearch(cell.X, cell.Y, Radius, IsOtherReachablePsychic)));
		if (list == null)
		{
			return;
		}
		...
		{
			GameObject gameObject = list[i];
			if (!gameObject.HasEffect("SensePsychicEffect", IsOurs))
			{
