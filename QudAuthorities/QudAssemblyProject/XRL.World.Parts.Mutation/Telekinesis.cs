...
public class Telekinesis : BaseMutation
{
	...
	{
		return GetTelekineticStrength() * 15;
	}

	public override string GetDescription()
	{
		return "You can move things with your mind.";
	}
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != GetThrowProfileEvent.ID && ID != InventoryActionEvent.ID)
		{
	...
	public override bool HandleEvent(OwnerGetInventoryActionsEvent E)
	{
		if (E.Object.IsReal)
		{
			...
			if ((E.Object.CurrentCell != null || E.Object.FireEvent("CanBeUnequipped")) && (E.Object.IsTakeable() || E.Object.IsCreature))
			{
	...
	public override bool HandleEvent(InventoryActionEvent E)
	{
		if (E.Command == "TelekineticPullAndTake" || E.Command == "TelekineticPullAndTakeOne" || E.Command == "TelekineticPull" || E.Command == "TelekineticPullOne")
		{
			...
			if (num2 > 1.0)
			{
				num3 = obj.Accelerate(Math.Min(GetTelekineticWeightCapacity(), (int)((num2 + 1.0) * (double)obj.GetKineticResistance())), null, ParentObject.CurrentCell, null, "Telekinetic", ParentObject, Accidental: true, null, null, 1.0, SuspendFalling: true, OneShort: true);
			}
			bool flag = false;
			if (GameObject.validate(obj) || obj.IsInGraveyard())
			{
				if (E.Command == "TelekineticPullAndTake" && obj.CurrentCell.DistanceTo(ParentObject) <= 1 && ParentObject.TakeObject(obj, Silent: false, 0))
				{
					flag = true;
				}
				else if (num3 > 0)
				...
				else if (obj.CurrentCell == cell && E.Actor.IsPlayer())
				{
					if (obj.IsPlayer())
					{
					...
					{
						Popup.ShowFail(obj.The + obj.ShortDisplayName + obj.GetVerb("do") + " not budge.");
					}
				}
			}
			else
			...
			if (GameObject.validate(ref obj) && obj.CurrentCell == cell && num3 == 0)
			{
				obj.CheckStack();
			}
		}
		else if (E.Command == "TelekineticHurl" || E.Command == "TelekineticHurlOne" || E.Command == "TelekineticMove" || E.Command == "TelekineticMoveOne")
		{
			...
			Cell cell2 = obj2.GetCurrentCell();
			if (cell2 != null)
			{
				Cell cell3 = PickTarget.ShowPicker(PickTarget.PickStyle.Line, 999, 999, cell2.X, cell2.Y, Locked: false, AllowVis.Any, null, null, null, cell2.Pos2D);
				if (cell3 != null)
				{
					if (flag4)
					{
					...
					if (obj2.CurrentCell != null || obj2.TryRemoveFromContext())
					{
						if (obj2.CurrentCell == null)
						{
							cell2.AddObject(obj2);
						}
						int num4 = GetTelekineticWeightCapacity();
						if (!flag2)
						{
							num4 = Math.Min(num4, ((int)cell2.RealDistanceTo(cell3) + 1) * obj2.GetKineticResistance());
						}
						int num5 = obj2.Accelerate(num4, null, cell3, null, "Telekinetic", ParentObject, Accidental: false, null, null, 1.0, SuspendFalling: true, OneShort: false, flag2);
						if (!GameObject.validate(obj2) || obj2.IsInGraveyard() || num5 > 0)
						{
							if (flag2)
							{
						...
						else if (obj2.CurrentCell == cell2)
						{
							if (E.Actor.IsPlayer())
							{
								if (obj2.IsPlayer())
								...
								{
									Popup.ShowFail(obj2.The + obj2.ShortDisplayName + obj2.GetVerb("do") + " not budge.");
								}
							}
						}
						else
						...
						if (GameObject.validate(ref obj2) && obj2.CurrentCell == cell2 && num5 == 0)
						{
							obj2.CheckStack();
						}
					}
	...
	public override void Register(GameObject Object)
	{
		...
		Object.RegisterPartEvent(this, "CommandAIDetonateExplosive");
		Object.RegisterPartEvent(this, "CommandAIHurlAt");
		Object.RegisterPartEvent(this, "CommandAIHurlInto");
		Object.RegisterPartEvent(this, "CommandTelekinesis");
		Object.RegisterPartEvent(this, "CommandToggleTelekineticThrowing");
	...
	public override bool FireEvent(Event E)
	{
		if (E.ID == "AIGetOffensiveMutationList")
		{
			...
			if (gameObjectParameter != null && IsMyActivatedAbilityVoluntarilyUsable(TelekinesisActivatedAbilityID) && ParentObject.HasLOSTo(gameObjectParameter, IncludeSolid: false))
			{
				if (FindExplosiveToDetonateNear(gameObjectParameter) != null)
				{
					E.AddAICommand("CommandAIDetonateExplosive");
				}
				if (FindHurlAtObjectNear(gameObjectParameter) != null)
				{
					E.AddAICommand("CommandAIHurlAt");
				}
				if (FindHurlIntoObjectNear(gameObjectParameter) != null)
				{
					E.AddAICommand("CommandAIHurlInto");
				}
			}
		}
		else if (E.ID == "CommandAIHurlAt")
		{
	...
	{
		TelekinesisActivatedAbilityID = GO.AddActivatedAbility("Telekinesis", "CommandTelekinesis", "Mental Mutation", "You can use this ability to choose an object to interact with telekinetically. You can also use telekinesis directly by interacting with an object via Look or Interact Nearby.\n\nWhen moving things telekinetically, \"move\" or \"pull\" indicates moving something gently, with as little force as needed, minimizing any injuries or damage from accidents. \"Hurl\" indicates moving something with maximum possible force.");
		TelekineticThrowingActivatedAbilityID = GO.AddActivatedAbility("Telekinetic Throwing", "CommandToggleTelekineticThrowing", "Mental Mutation", "Normally, one uses one's telekinesis to augment throwing. Toggle this ability to control this behavior.", "\a", null, Toggleable: true, DefaultToggleState: true);
	...
	public bool AttemptTelekinesis()
	{
		...
		while (true)
		{
			Cell cell2 = PickTarget.ShowPicker(PickTarget.PickStyle.EmptyCell, telekineticRange, telekineticRange, x, y, Locked: false, AllowVis.OnlyVisible);
			if (cell2 == null)
			{
				return false;
			}
