...
public class Customs_Sharer : BaseSkill
{
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != GetNamingChanceEvent.ID && ID != GetNamingBestowalChanceEvent.ID && ID != GetRitualSifrahSetupEvent.ID)
		{
			return ID == GetWaterRitualSellSecretBehaviorEvent.ID;
		}
		return true;
	}
