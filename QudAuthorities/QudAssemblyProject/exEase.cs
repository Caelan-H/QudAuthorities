...
public class exEase
{
	...
	public static float SineIn(float _t)
	{
		if (_t != 1f)
		{
			return 0f - Mathf.Cos(_t * (float)Math.PI / 2f) + 1f;
		}
		return 1f;
	}
	...
	{
		return Mathf.Sin(_t * (float)Math.PI / 2f);
	}

	public static float SineInOut(float _t)
	{
		return -0.5f * (Mathf.Cos((float)Math.PI * _t) - 1f);
	}
	...
	private static float ElasticInHelper(float _t, float _b, float _c, float _d, float _a, float _p)
	{
		...
		{
			num2 = _p / 2f * (float)Math.PI * Mathf.Asin(_c / _a);
		}
		num -= 1f;
		return 0f - _a * Mathf.Pow(2f, 10f * num) * Mathf.Sin((num * _d - num2) * 2f * (float)Math.PI / _p) + _b;
	}
	...
	private static float ElasticOutHelper(float _t, float _b, float _c, float _d, float _a, float _p)
	{
		...
		{
			num = _p / 2f * (float)Math.PI * Mathf.Asin(_c / _a);
		}
		return _a * Mathf.Pow(2f, -10f * _t) * Mathf.Sin((_t - num) * 2f * (float)Math.PI / _p) + _c;
	}
	...
	public static float ElasticInOut(float _t, float _a, float _p)
	{
		...
		{
			num = _p / 2f * (float)Math.PI * Mathf.Asin(1f / _a);
		}
		if (_t < 1f)
		{
			return -0.5f * (_a * Mathf.Pow(2f, 10f * (_t - 1f)) * Mathf.Sin((_t - 1f - num) * 2f * (float)Math.PI / _p));
		}
		return _a * Mathf.Pow(2f, -10f * (_t - 1f)) * Mathf.Sin((_t - 1f - num) * 2f * (float)Math.PI / _p) * 0.5f + 1f;
	}
	...
	{
		_t = Mathf.Clamp01(_t);
		_t = (Mathf.Sin(_t * (float)Math.PI * (0.2f + 2.5f * _t * _t * _t)) * Mathf.Pow(1f - _t, 2.2f) + _t) * (1f + 1.2f * (1f - _t));
		return _t;
	}
	...
	public static float Punch(float _amplitude, float _t)
	{
		...
		return _amplitude * Mathf.Pow(2f, -10f * _t) * Mathf.Sin((_t * 1f - num) * ((float)Math.PI * 2f) / num2);
	}
