...
public class EmbarkBuilderModuleWindowDescriptor
{
	...
	public bool enabled
	{
		get
		{
			...
			if ((object)abstractBuilderModuleWindowBase == null || abstractBuilderModuleWindowBase.enabled)
			{
		...
		if (window == null)
		{
			...
			else
			{
				...
				if (gameObject != null)
				{
					...
					window.name = name;
					UnityEngine.Object.Instantiate(gameObject).transform.SetParent(window.transform, worldPositionStays: false);
				}
			}
		}
		if (window == null)
