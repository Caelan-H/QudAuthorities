...
public class TBInputManager : MonoBehaviour
{
	...
	public bool ProjectScreenPointOnDragPlane(Vector3 refPos, Vector2 screenPos, out Vector3 worldPos)
	{
		...
		case DragPlaneType.UseCollider:
		{
			...
			if (!dragPlaneCollider.Raycast(ray2, out var hitInfo, float.MaxValue))
			{
				return false;
			}
	...
	private void draggable_OnDragMove(TBDrag sender)
	{
		...
		if (ProjectScreenPointOnDragPlane(sender.transform.position, screenPos, out var worldPos) && ProjectScreenPointOnDragPlane(sender.transform.position, sender.FingerPos, out var worldPos2))
		{
	...
	{
		if (Physics.Raycast(raycastCamera.ScreenPointToRay(screenPos), out var hitInfo, float.MaxValue, ~(int)ignoreLayers))
		{
