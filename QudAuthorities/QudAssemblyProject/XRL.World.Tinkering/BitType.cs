...
public class BitType
{
	...
	{
		Array.Sort(Bits, (char a, char b) => BitSortOrder[a].CompareTo(BitSortOrder[b]));
	}

	public static void SortBits(List<char> Bits)
	{
		Bits.Sort((char a, char b) => BitSortOrder[a].CompareTo(BitSortOrder[b]));
	}

	public static void SortBits(List<string> Bits)
	{
		Bits.Sort((string a, string b) => BitSortOrder[a[0]].CompareTo(BitSortOrder[b[0]]));
	}
	...
	private static void AddType(BitType NewType, bool TierPriority = false)
	{
