...
public static class Keyboard
{
	public class XRLKeyEvent
	{
		...
		public XRLKeyEvent(Event ke)
		{
			...
			control = ke.control || Input.GetKey(UnityEngine.KeyCode.RightControl) || Input.GetKey(UnityEngine.KeyCode.LeftControl) || Input.GetKey(UnityEngine.KeyCode.RightCommand) || Input.GetKey(UnityEngine.KeyCode.LeftCommand);
			alt = ke.alt;
		}
	...
	public static char ConvertKeycodeToLowercaseChar(UnityEngine.KeyCode code)
	{
		...
		MetricsManager.LogError("uncovertable unity key code " + code);
		return '\0';
	}
	...
	public static void ClearMouseEvents()
	{
		lock (MouseEventQueue)
		{
			...
			{
				MouseEvent mouseEvent = MouseEventUsed.Dequeue();
				mouseEvent.Data = null;
				MouseEventPool.Enqueue(mouseEvent);
			}
			while (MouseEventQueue.Count > 0)
			{
				MouseEvent mouseEvent2 = MouseEventQueue.Dequeue();
				mouseEvent2.Data = null;
				MouseEventPool.Enqueue(mouseEvent2);
			}
		}
	}
	...
	public static void GetNextKey(bool MapDirectionsToArrows, bool pumpActions = false, bool waitForNext = true)
	{
		...
		while (true)
		{
			...
			if (vkCode == (Keys)65726)
			{
				Char = 62;
			}
			if (vkCode == (Keys)65727)
			{
				Char = 63;
			}
			if (vkCode == (Keys)65725)
			{
				Char = 95;
			}
			if (vkCode == (Keys)65722)
			...
			if (vkCode == (Keys)65723)
			{
				Char = 43;
			}
			if (vkCode == (Keys)65724)
			{
				Char = 60;
			}
			if (vkCode == (Keys)65728)
			{
				Char = 126;
			}
			if (vkCode == (Keys)65755)
			{
				Char = 123;
			}
			if (vkCode == (Keys)65756)
			{
				Char = 124;
			}
			if (vkCode == (Keys)65757)
			{
				Char = 125;
			}
			if (vkCode == (Keys)65758)
			{
				Char = 34;
			}
			if (vkCode == (Keys)65762)
			{
				Char = 124;
			}
			if (vkCode == Keys.Left)
			...
			MetaKey = (int)vkCode;
			if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdIgnore")
			{
				continue;
			}
			if (MapDirectionsToArrows && xRLKeyEvent.allowmap)
			{
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdWait")
				{
					vkCode = Keys.NumPad5;
					Char = 53;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveN")
				{
					vkCode = Keys.NumPad8;
					Char = 56;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveS")
				{
					vkCode = Keys.NumPad2;
					Char = 50;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveE")
				{
					vkCode = Keys.NumPad6;
					Char = 54;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveW")
				{
					vkCode = Keys.NumPad4;
					Char = 52;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveNW")
				{
					vkCode = Keys.NumPad7;
					Char = 55;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveNE")
				{
					vkCode = Keys.NumPad9;
					Char = 57;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveSW")
				{
					vkCode = Keys.NumPad1;
					Char = 49;
				}
				if (LegacyKeyMapping.MapKeyToCommand(MetaKey) == "CmdMoveSE")
				{
					vkCode = Keys.NumPad3;
					Char = 51;
				}
			...
			if (Options.GetOption("OptionMapShiftDirectionToPage") == "Yes" && xRLKeyEvent.shift)
			{
				...
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdWait")
				{
					vkCode = Keys.NumPad5;
					Char = 53;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveN")
				{
					vkCode = Keys.NumPad8;
					Char = 56;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveS")
				{
					vkCode = Keys.NumPad2;
					Char = 50;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveE")
				{
					vkCode = Keys.NumPad6;
					Char = 54;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveW")
				{
					vkCode = Keys.NumPad4;
					Char = 52;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveNW")
				{
					vkCode = Keys.NumPad7;
					Char = 55;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveNE")
				{
					vkCode = Keys.NumPad9;
					Char = 57;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveSW")
				{
					vkCode = Keys.NumPad1;
					Char = 49;
				}
				if (LegacyKeyMapping.MapKeyToCommand(num) == "CmdMoveSE")
				{
					vkCode = Keys.NumPad3;
					Char = 51;
				}
	...
	{
		string text = "";
		if (((uint)MetaCode & 0x20000u) != 0)
		{
			text += "Ctrl-";
		}
		if (((uint)MetaCode & 0x40000u) != 0)
		{
			text += "Alt-";
		}
		if (((uint)MetaCode & 0x10000u) != 0)
		{
			text += "Shift-";
		}
		MetaCode &= 0xFFFF;
