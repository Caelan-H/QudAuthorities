...
public static class MarkupShaders
{
	...
	private static void Init()
	{
		...
		foreach (char key in ColorUtility.ColorMap.Keys)
		{
			...
			Register(new SolidColor("^" + key, 'y', key));
		...
		{
			ProcessColorsXmlFile("Colors.xml", null);
			ModManager.ForEachFile("Colors.xml", delegate(string file, ModInfo modInfo)
			{
				ProcessColorsXmlFile(file, modInfo);
			});
		});
	}

	public static void ProcessColorsXmlFile(string file, ModInfo modInfo)
	{
		using XmlDataHelper xmlDataHelper = DataManager.GetXMLStream(file, modInfo);
		xmlDataHelper.HandleNodes(_outerNodes);
		xmlDataHelper.Close();
	}
