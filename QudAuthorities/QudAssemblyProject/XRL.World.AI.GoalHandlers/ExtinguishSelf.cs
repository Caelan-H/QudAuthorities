...
public class ExtinguishSelf : GoalHandler
{
	...
	{
		if (GameObject.validate(ref Target) && Target.IsInGraveyard())
		{
			Target = null;
		}
		...
		if (num > 7 && Stat.Random(7, 25) < num)
		{
			...
			if (gameObject != null)
			{
				...
				float liquidCooling = liquidVolume.GetLiquidCooling();
				if (liquidVolume.Pour(PourAmount: (int)Math.Ceiling((float)(base.ParentObject.pPhysics.Temperature / base.ParentObject.pPhysics.FlameTemperature) / liquidCooling), Actor: base.ParentObject, TargetCell: null, Forced: false, Douse: true))
				{
					return;
				}
			}
