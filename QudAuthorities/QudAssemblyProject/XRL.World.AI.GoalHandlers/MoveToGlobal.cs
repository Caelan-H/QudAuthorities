...
public class MoveToGlobal : IMovementGoal
{
	...
	public override void TakeAction()
	{
		...
		if (base.ParentObject.InZone(dZone))
		{
			...
			FindPath findPath = new FindPath(base.ParentObject.CurrentZone.ZoneID, base.ParentObject.CurrentCell.X, base.ParentObject.CurrentCell.Y, dZone, dCx, dCy, PathGlobal: true, PathUnlimited: false, base.ParentObject);
			if (findPath.bFound)
			{
		...
		if (base.ParentObject.CurrentZone.Z < num4)
		{
			...
			if (gameObject != null)
			{
				...
				base.ParentObject.pBrain.PushGoal(new MoveTo(gameObject, careful: false, OverridesCombat));
				return;
			}
		}
		if (base.ParentObject.CurrentZone.Z > num4)
		{
			...
			if (gameObject2 != null)
			{
				...
				base.ParentObject.pBrain.PushGoal(new MoveTo(gameObject2, careful: false, OverridesCombat));
				return;
			}
		}
		...
		for (int m = 0; m < list2.Count; m++)
		{
			...
			FindPath findPath2 = new FindPath(ParentBrain.pPhysics.CurrentCell.ParentZone.ZoneID, ParentBrain.pPhysics.CurrentCell.X, ParentBrain.pPhysics.CurrentCell.Y, ParentBrain.pPhysics.CurrentCell.ParentZone.ZoneID, list2[m].X, list2[m].Y, PathGlobal: true, PathUnlimited: false, base.ParentObject);
			if (!findPath2.bFound)
			{
				break;
			}
