...
public static class GenotypeFactory
{
	...
	public static void Init()
	{
		...
		using (XmlDataHelper xmlDataHelper = DataManager.GetXMLStream("Genotypes.xml", null))
		{
			HandleNodes(xmlDataHelper);
			xmlDataHelper.Close();
		}
		ModManager.ForEachFile("Genotypes.xml", delegate(string File, ModInfo ModInfo)
		{
			using XmlDataHelper xmlDataHelper2 = DataManager.GetXMLStream(File, ModInfo);
	...
	public static GenotypeEntry LoadGenotypeNode(XmlDataHelper Reader, bool bMod)
	{
		...
		NewGenotype.Code = Reader.GetAttribute("Code");
	...
	public static void LoadSkillNode(GenotypeEntry NewGenotype, XmlDataHelper Reader, bool bMod)
	{
		...
		if (!NewGenotype.Skills.Contains(attribute))
		{
			NewGenotype.Skills.Add(attribute);
		}
		Reader.DoneWithElement();
	}
	...
	{
		string attribute = Reader.GetAttribute("Name");
		if (!NewGenotype.RemoveSkills.Contains(attribute))
		{
			NewGenotype.RemoveSkills.Add(attribute);
		}
		Reader.DoneWithElement();
	}
