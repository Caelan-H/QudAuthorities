...
public abstract class SifrahGame
{
	...
	public string CriticalFailureSound = "death_robot_1";

	public string FailureSound = "sunder_mind_fail";

	public string PartialSuccessSound = "refract";

	public string SuccessSound = "startup";

	public string CriticalSuccessSound = "grenade_flash";
	...
	public virtual string[] GetStatus()
	{
		...
		status[0] = "Chances:";
		status[1] = " {{g|Success             " + string.Format("{0,3}%", Success);
		status[2] = " {{G|Exceptional success " + string.Format("{0,3}%", CriticalSuccess);
		status[3] = " {{W|Partial success     " + string.Format("{0,3}%", PartialSuccess);
		status[4] = " {{K|Failure             " + string.Format("{0,3}%", Failure);
		status[5] = " {{R|Critical failure    " + string.Format("{0,3}%", CriticalFailure);
		return status;
	}
	...
	public void ProcessTurn(GameObject ContextObject)
	{
		...
		for (int count4 = Slots.Count; l < count4; l++)
		{
			...
			if (slot.Solved)
			{
				if (slot.SolvedOnTurn != Turn || string.IsNullOrEmpty(CorrectTokenSound) || !Options.Sound)
				{
					continue;
				}
				if (num > 0)
			...
			if (slot.CurrentMove != -1 && !string.IsNullOrEmpty(IncorrectTokenSound) && Options.Sound)
			{
				if (num > 0)
				{
					Task.Delay(num).ContinueWith(delegate
	...
	public bool MakeMoveForSlot(int SlotNumber, GameObject ContextObject)
	{
		...
		while (true)
		{
			...
			string title = "Use which option for " + sifrahSlot.Description + "?";
			IRenderable[] icons = array3;
			int num2 = Popup.ShowOptionList(title, array, array2, 0, null, 60, RespectOptionNewlines: false, AllowEscape: true, num, "", null, null, icons);
			if (num2 < 0)
			{
				break;
			}
	...
	{
		BookUI.ShowBook("Sifrah");
	}

	public virtual bool CheckIncompleteTurn(GameObject ContextObject)
	{
		return Popup.ShowYesNo("You haven't selected an option for every slot. Are you sure you want to complete the turn?", AllowEscape: true, DialogResult.No) == DialogResult.Yes;
	}

	public virtual bool CheckEarlyExit(GameObject ContextObject)
	{
		return Popup.ShowYesNo("You aren't finished! Are you sure you want to abort the process?", AllowEscape: true, DialogResult.No) == DialogResult.Yes;
	}
	...
	public void Play(GameObject ContextObject = null)
	{
		...
		while (!flag)
		{
			...
			else if (keys == Keys.Enter && Slots != null && SlotSelected >= 0 && SlotSelected < Slots.Count)
			{
				num = SlotSelected;
			}
			else if (keys == Keys.Space || (keys == Keys.Z && Options.PrereleaseInputManager))
			{
	...
	public void UseInsight(GameObject ContextObject)
	{
		...
		if (list != null && list.Count > 0)
		{
			...
			Popup.Show(stringBuilder.ToString(), CopyScrap: true, Capitalize: true, DimBackground: true, LogMessage: false);
	...
	protected static void RecordOutcome(string GameType, GameOutcome Outcome, int Slots, int Tokens, bool? AsMaster = null, bool? Mastered = null)
	{
		...
		if (AsMaster.HasValue || Mastered.HasValue)
		{
			...
			IncrementGameCount(GameType, null, null, null, null, Mastered);
			IncrementGameCount(GameType, null, null, null, AsMaster);
			IncrementGameCount(GameType, Outcome, null, null, AsMaster, Mastered);
			IncrementGameCount(GameType, Outcome, null, null, null, Mastered);
			IncrementGameCount(GameType, Outcome, null, null, AsMaster);
			IncrementGameCount(GameType, null, Slots, Tokens, AsMaster, Mastered);
			IncrementGameCount(GameType, null, Slots, Tokens, null, Mastered);
			IncrementGameCount(GameType, null, Slots, Tokens, AsMaster);
