...
public static class ZoneTemplateManager
{
	...
	private static void LoadTemplates()
	{
		...
		Paths.Add(DataManager.FilePath("ZoneTemplates.xml"));
		Paths.AddRange(Directory.GetFiles(DataManager.FilePath("."), "ZoneTemplates_*.xml", SearchOption.AllDirectories));
		ModManager.ForEachFile("ZoneTemplates.xml", delegate(string path)
		{
			Paths.Add(path);
		});
		foreach (string item in Paths)
		{
			...
			{
				if (xmlTextReader.NodeType == XmlNodeType.Element && xmlTextReader.Name == "zonetemplate")
				{
					ZoneTemplate zoneTemplate = LoadTemplate(xmlTextReader);
