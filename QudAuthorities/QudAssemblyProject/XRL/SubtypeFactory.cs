...
public static class SubtypeFactory
{
	...
	public static void Init()
	{
		...
		using (XmlTextReader xmlTextReader = DataManager.GetStreamingAssetsXMLStream("Subtypes.xml"))
		{
			xmlTextReader.WhitespaceHandling = WhitespaceHandling.None;
			while (xmlTextReader.Read())
			{
				if (xmlTextReader.Name == "subtypes")
				{
					LoadSubtypesNode(xmlTextReader);
				}
			}
			xmlTextReader.Close();
		}
		ModManager.ForEachFile("Subtypes.xml", delegate(string s)
		{
			using XmlTextReader xmlTextReader2 = new XmlTextReader(s);
			xmlTextReader2.WhitespaceHandling = WhitespaceHandling.None;
			while (xmlTextReader2.Read())
			{
				if (xmlTextReader2.Name == "subtypes")
				{
					LoadSubtypesNode(xmlTextReader2, bMod: true);
				}
				if (xmlTextReader2.NodeType == XmlNodeType.EndElement && xmlTextReader2.Name == "subtypes")
				{
					break;
				}
			}
		...
		while (Reader.Read())
		{
			if (Reader.Name == "class")
			{
				if (Reader.NodeType == XmlNodeType.Element)
				{
					...
					subtypeClass.SingluarTitle = Reader.GetAttribute("SingularTitle") ?? subtypeClass.ChargenTitle;
	...
	public static void LoadSubtypeNode(SubtypeClass currentClass, SubtypeCategory currentCategory, XmlTextReader Reader, bool bMod)
	{
		...
		subtypeEntry.Code = Reader.GetAttribute("Code");
	...
	{
		string attribute = Reader.GetAttribute("Name");
		if (!NewSubtype.Skills.Contains(attribute))
		{
			NewSubtype.Skills.Add(attribute);
		}
		if (Reader.NodeType != XmlNodeType.EndElement && !Reader.IsEmptyElement)
		{
	...
	{
		string attribute = Reader.GetAttribute("Name");
		if (!NewSubtype.RemoveSkills.Contains(attribute))
		{
			NewSubtype.RemoveSkills.Add(attribute);
		}
		if (Reader.NodeType != XmlNodeType.EndElement && !Reader.IsEmptyElement)
		{
