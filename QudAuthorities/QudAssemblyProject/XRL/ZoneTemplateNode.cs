	...
	public virtual bool TestCriteria(ZoneTemplateGenerationContext Context)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (text.StartsWith("regionhassemantictag:"))
			{
				...
				if (!Context.currentSector.Cells.Any((Location2D loc) => tags2.Any((string tag) => Context.Z.GetCell(loc).HasSemanticTag(tag))))
				{
					return false;
				}
				continue;
			...
			if (text.StartsWith("zonehassemantictag:"))
			{
				...
				if (!Context.Z.GetSemanticTags().Any((string t) => tags.Contains(t.ToLower())))
				{
					return false;
				}
				continue;
