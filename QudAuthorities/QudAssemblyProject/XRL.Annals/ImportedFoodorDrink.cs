...
public class ImportedFoodorDrink : HistoricEvent
{
	...
	public ImportedFoodorDrink(bool _bVillageZero)
	{
	...
	public override void Generate()
	{
		...
		if (num <= 334)
		{
			...
			text = text.TrimEnd(',');
			LiquidVolume liquidVolume = GameObject.create("Waterskin").GetPart("LiquidVolume") as LiquidVolume;
			liquidVolume.ComponentLiquids.Clear();
			liquidVolume.InitialLiquid = text;
			string text3 = ColorUtility.StripFormatting(liquidVolume.GetLiquidName());
			addEntityListItem("signatureLiquids", liquidVolume.GetLiquidDesignation());
		...
		if (num <= num2 + 333)
		{
			...
			string text7 = (gameObject.HasPart("LiquidVolume") ? ColorUtility.StripFormatting((gameObject.GetPart("LiquidVolume") as LiquidVolume).GetPrimaryLiquid().GetName(null)) : gameObject.DisplayNameOnlyDirectAndStripped);
