...
public class ChariotDrivesOffCliff : HistoricEvent
{
	...
	public override void Generate()
	{
		...
		if (50.in100())
		{
			...
			text2 = Grammar.MakePossessive(snapshotAtYear.GetProperty("name")) + " chariot was driven off a cliff by <spice.commonPhrases.rogue.!random.article> <spice.commonPhrases.band.!random> of " + Faction.getFormattedName(text);
			text3 = ExpandString("<spice.commonPhrases.band.!random.article> of treacherous " + Faction.getFormattedName(text) + " <spice.history.gospels.VehicularSabotage." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".!random> in " + Grammar.MakePossessive(snapshotAtYear.GetProperty("name")) + " <spice.history.gospels." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".vehicle.!random>") + ", causing it to " + ExpandString("<spice.history.gospels.VehicularSabotageResult." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".!random>");
			text4 = ExpandString("<spice.commonPhrases.band.!random.article> of treacherous " + Faction.getFormattedName(text) + " <spice.history.gospels.VehicularSabotage." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".!random> in " + ExpandString("<entity.possessivePronoun>") + " <spice.history.gospels." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".vehicle.!random>") + ", causing it to " + ExpandString("<spice.history.gospels.VehicularSabotageResult." + QudHistoryHelpers.GetSultanateEra(snapshotAtYear) + ".!random>");
		}
		else
		{
