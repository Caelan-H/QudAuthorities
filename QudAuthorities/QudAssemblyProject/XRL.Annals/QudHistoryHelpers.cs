...
public static class QudHistoryHelpers
{
	...
	public static string GetNewLocationInRegion(History history, string region, string currentLocation)
	{
		...
		newEntity.ApplyEvent(new InitializeLocation(region, 0));
		return newEntity.GetCurrentSnapshot().GetProperty("name");
	}
	...
	public static string Ansify(string name)
	{
		...
		return ComposePattern(text, list3, patternType);
	}
	...
	public static string ConvertGospelToSultanateCalendarEra(string gospel, int endOfSultanateDate)
	{
		if (!gospel.Contains("%"))
		{
			return gospel;
		...
		if (match.Success)
		{
			return Regex.Replace(gospel, "\\%.*?\\%", ConvertYearToSultanateCalendarEra(int.Parse(match.Groups[1].Value), endOfSultanateDate));
		}
		return gospel;
	}

	public static string ConvertYearToSultanateCalendarEra(int year, int endOfSultanateDate)
	{
