...
public class ParticleManager
{
	...
	{
		AddRadial(Text, x, y, r, d, rdel, ddel, 999);
	}

	public void Add(string Text, float x, float y, float xDel, float yDel)
	{
		Add(Text, x, y, xDel, yDel, 999, 0f, 0f);
	}

	public void Add(string Text, float x, float y, float xDel, float yDel, int Life)
	{
		Add(Text, x, y, xDel, yDel, Life, 0f, 0f);
	}

	public void Add(string Text, float x, float y, float xDel, float yDel, int Life, float xDelDel, float yDelDel)
	{
	...
	public void AddTile(string Tile, string ColorString, string DetailColor, float x, float y, float xDel, float yDel, int Life, float xDelDel, float yDelDel, bool HFlip = false, bool VFlip = false)
	{
		if (Particles.Count <= 1000)
		{
	...
	public void AddSinusoidal(string Text, float x, float y, float amp, float sinstart, float sinincrement, float xamp, float yamp, float xdel, float ydel, int Life)
	{
	...
	public void AddRadial(string Text, float x, float y, float r, float d, float rdel, float ddel, int Life)
	{
	...
	public void Render(ScreenBuffer Buffer)
	{
		...
		for (int i = 0; i < Particles.Count; i++)
		{
			...
			if (textParticle.Text == "firefly")
			{
				...
				{
					Buffer.Buffer[num, num2].Attributes = ColorUtility.MakeColor(TextColor.Blue, TextColor.Black);
				}
				else if (textParticle.Life < 20)
				{
					Buffer.Buffer[num, num2].Attributes = ColorUtility.MakeColor(ColorUtility.Bright(TextColor.Blue), TextColor.Black);
				}
				else if (textParticle.Life < 30)
				{
					Buffer.Buffer[num, num2].Attributes = ColorUtility.MakeColor(TextColor.Cyan, TextColor.Black);
				}
				else if (textParticle.Life < 40)
				{
					Buffer.Buffer[num, num2].Attributes = ColorUtility.MakeColor(ColorUtility.Bright(TextColor.Cyan), TextColor.Black);
				}
		...
		for (int j = 0; j < TileParticles.Count; j++)
		{
			...
			if (!string.IsNullOrEmpty(tileParticle.ColorString))
			{
				...
				Buffer.Buffer[num5, num6].Attributes = ColorUtility.MakeColor(foreground, background);
			}
		...
		for (int k = 0; k < RadialParticles.Count; k++)
		{
			...
			if (radialTextParticle.Text[0] == '@')
			{
				...
				if ((float)num9 != radialTextParticle.x || (float)num10 != radialTextParticle.y)
				{
					...
					Buffer.Buffer[num9, num10].Attributes = Buffer.Buffer[num11, num12].Attributes;
				}
			}
			else
			{
		...
		for (int l = 0; l < SinusoidalParticles.Count; l++)
		{
			...
			if (sinusoidalTextParticle.Text[0] == '@')
			{
				...
				if ((float)num13 != sinusoidalTextParticle.x || (float)num14 != sinusoidalTextParticle.y)
				{
					...
					Buffer.Buffer[num13, num14].Attributes = Buffer.Buffer[num15, num16].Attributes;
				}
			}
			else
			{
				...
				foreach (string line2 in bannerText.Block.Lines)
				{
					...
					for (int m = 0; m < text.Length; m++)
					{
						...
						else
						{
							...
							SB.Append("*");
							Buffer.Write(SB.ToString());
							AddRadial("&Y.", Buffer.X, Buffer.Y, Stat.RandomCosmetic(0, 5), Stat.RandomCosmetic(5, 10), 0f, 0.05f * (float)Stat.RandomCosmetic(3, 7), 15);
						}
					}
					num18++;
				}
