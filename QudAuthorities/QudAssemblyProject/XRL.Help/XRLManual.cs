...
public class XRLManual
{
	...
	public XRLManual(TextConsole _Console)
	{
		...
		Pages.Clear();
		XmlTextReader streamingAssetsXMLStream = DataManager.GetStreamingAssetsXMLStream("Manual.xml");
		streamingAssetsXMLStream.WhitespaceHandling = WhitespaceHandling.None;
		while (streamingAssetsXMLStream.Read())
		{
			if (streamingAssetsXMLStream.Name == "topic" && streamingAssetsXMLStream.NodeType != XmlNodeType.EndElement)
			{
				string text = streamingAssetsXMLStream["name"];
				XRLManualPage xRLManualPage = new XRLManualPage(streamingAssetsXMLStream.ReadString())
				{
					Topic = text
				};
	...
	public void ShowHelp(string DefaultPage)
	{
		...
		while (true)
		{
			...
			Keys keys = ConsoleLib.Console.Keyboard.getvk(MapDirectionToArrows: true);
			if (CurrentPage == "")
			{
				...
				if (keys == Keys.Next || keys == Keys.Next || keys == Keys.NumPad3 || ConsoleLib.Console.Keyboard.RawCode == Keys.Next || ConsoleLib.Console.Keyboard.RawCode == Keys.Next)
				{
					IndexPosition += 23;
				}
				...
				if (keys == Keys.Prior || keys == Keys.Back || keys == Keys.NumPad9 || ConsoleLib.Console.Keyboard.RawCode == Keys.Back || ConsoleLib.Console.Keyboard.RawCode == Keys.Prior)
				{
					IndexPosition -= 23;
				}
				if (keys == Keys.End)
			...
			if ((keys == Keys.Escape || keys == Keys.NumPad5 || (keys == Keys.MouseEvent && ConsoleLib.Console.Keyboard.CurrentMouseEvent.Event == "RightClick")) && CurrentPage == "")
			{
				break;
			}
			if ((keys == Keys.Escape || keys == Keys.NumPad5 || keys == Keys.Escape || keys == Keys.Enter || (keys == Keys.MouseEvent && ConsoleLib.Console.Keyboard.CurrentMouseEvent.Event == "RightClick")) && CurrentPage != "")
			{
				CurrentPage = "";
			}
			else
