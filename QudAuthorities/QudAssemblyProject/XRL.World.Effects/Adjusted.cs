...
public class Adjusted : Effect
{
		...
		for (; i < array.Length; i++)
		{
			...
			case "Animate":
				...
				case "DetailColor":
					...
					break;
				case "FirstFrame":
					AnimateFirstFrame = num;
					break;
				case "LastFrame":
					AnimateLastFrame = num;
					break;
				default:
	...
	{
		if (Animate && base.Duration > 0 && XRLCore.CurrentFrame >= AnimateFirstFrame && XRLCore.CurrentFrame < AnimateLastFrame)
		{
	...
	public override bool Apply(GameObject Object)
	{
		...
		{
			foreach (Adjusted effect in Object.GetEffects(typeof(Adjusted)))
			{
				if (effect != null && effect.Spec == Spec)
				{
					if (Refreshes && effect.Duration < base.Duration)
					{
		...
		if (Object.FireEvent(Event.New("ApplyAdjusted", "Effect", this, "Duration", base.Duration)))
		{
			ApplyAdjustments();
			return true;
		}
	...
	private void ApplyAdjustments()
	{
		if (StatAdjustOrder != null)
		{
			foreach (string item in StatAdjustOrder)
			{
				base.StatShifter.SetStatShift(item, StatAdjusts[item]);
			}
		}
		if (PropertyAdjustOrder == null)
		{
