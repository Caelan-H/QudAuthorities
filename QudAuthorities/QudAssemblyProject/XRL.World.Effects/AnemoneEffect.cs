...
public class AnemoneEffect : Effect
{
	...
	{
		base.DisplayName = "{{g|anticoagulant}}";
	}
	...
	{
		if (!base.WantEvent(ID, cascade) && ID != EndTurnEvent.ID && (ID != BeforeApplyDamageEvent.ID || DamageBonus == null))
		{
	...
	{
		if (Object.GetEffect("AnemoneEffect") is AnemoneEffect anemoneEffect)
		{
			anemoneEffect.Duration += base.Duration;
			return false;
		}
		return true;
	}
