...
public class ProceduralCookingEffectUnitMutation<T> : ProceduralCookingEffectUnit where T : BaseMutation, new()
{
	...
	public override string GetDescription()
	{
		...
		if (Tier == 1)
		{
			return "+" + Tier + " level to " + MutationDisplayName + ".";
		}
		if (Tier == 0)
		{
			return GetBonusZeroMessage();
		}
		return "+" + Tier + " levels to " + MutationDisplayName + ".";
	}
	...
	public override void Apply(GameObject go, Effect parent)
	{
		...
		modTracker = go.RequirePart<Mutations>().AddMutationMod(name, Tier, Mutations.MutationModifierTracker.SourceType.Cooking, parent.DisplayName);
