...
[UIView("EmbarkBuilder:Overlay", false, false, false, null, null, false, 0, false, NavCategory = "Menu", UICanvas = "Chargen/Overlay", UICanvasHost = 1)]
public class EmbarkBuilderOverlayWindow : WindowBase, ControlManager.IControllerChangedEvent
{
	...
	{
		Id = "Next",
		InputCommand = "NavigationPageYAxis+",
		KeyDescription = "End",
		Description = "Next"
	};
	...
	{
		RectTransform rectTransform = currentWindowDescriptor?.getWindow().safeArea;
		if (rectTransform != null)
		{
			rectTransform.anchorMin = safeArea.anchorMin;
			rectTransform.anchorMax = safeArea.anchorMax;
			rectTransform.anchoredPosition = safeArea.anchoredPosition;
			rectTransform.sizeDelta = safeArea.sizeDelta;
		}
	}
	...
	{
		if ((currentWindowDescriptor?.getWindow()?.isActiveAndEnabled).GetValueOrDefault())
		{
	...
	public void Update()
	{
		...
		if (base.rectTransform.rect.width != _lastWidth || base.rectTransform.rect.height != _lastHeight)
		{
			...
			safeArea.offsetMin = new Vector2((!breakNavButtons) ? 150 : 0, breakKeyDescriptions ? 50 : 100);
			safeArea.offsetMax = new Vector2((!breakNavButtons) ? (-150) : 0, safeArea.offsetMax.y);
			UpdateSafeArea();
		}
	}
