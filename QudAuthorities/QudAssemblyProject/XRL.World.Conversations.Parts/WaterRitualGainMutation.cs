...
public class WaterRitualGainMutation : IWaterRitualPart
{
	...
	public override bool HandleEvent(EnteredElementEvent E)
	{
		if (UseReputation())
		{
			Popup.Show("Despite your genetic limitations, " + The.Speaker.does("teach", int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true) + " you to improvise {{M|" + DisplayName + "}}!");
			Mutations mutations = The.Player.RequirePart<Mutations>();
			if (Entry != null)
			{
				mutations.AddMutation(Entry, 1);
			}
			else
			{
	...
	{
		E.Tag = "{{" + Lowlight + "|[gain {{M|" + DisplayName + "}}: {{" + Numeric + "|-" + Reputation + "}} reputation]}}";
		return false;
	}
}
