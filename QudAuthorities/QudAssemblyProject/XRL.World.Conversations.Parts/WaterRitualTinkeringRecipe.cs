...
public class WaterRitualTinkeringRecipe : IWaterRitualPart
{
	...
	public override bool HandleEvent(PrepareTextEvent E)
	{
		...
		else
		{
			...
			E.Text.Replace("=recipe=", gameObject.IsPluralIfKnown ? Data.DisplayName : Grammar.Pluralize(Data.DisplayName));
	...
	public override bool HandleEvent(EnteredElementEvent E)
	{
		if (UseReputation())
		{
			...
			if (Data.Type == "Mod")
			{
				Popup.Show(The.Speaker.Does("teach", int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true) + " you to craft the item modification {{W|" + Data.DisplayName + "}}.");
			}
			else
			{
				GameObject gameObject = GameObject.createSample(Data.Blueprint);
				gameObject.MakeUnderstood();
				Popup.Show(The.Speaker.Does("teach", int.MaxValue, null, null, AsIfKnown: false, Single: false, NoConfusion: false, NoColor: false, Stripped: true) + " you to craft " + (gameObject.IsPlural ? Data.DisplayName : Grammar.Pluralize(Data.DisplayName)) + ".");
				gameObject.Obliterate();
			}
		}
		return base.HandleEvent(E);
