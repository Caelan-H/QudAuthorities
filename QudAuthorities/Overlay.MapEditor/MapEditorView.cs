...
public class MapEditorView : BaseView
{
	...
	public void LoadMap(string file)
	{
		...
		FileName = file;
		GetChild("Title").rootObject.GetComponent<Text>().text = "Map Editor - " + FileName;
		RenderMap();
	...
	public void SetBrush(MapFileRegion brush)
	{
		...
		FindChild("SelectionHeader/SelectedItem").GetComponent<Text>().text = $"Brush: {GetBrushText()}";
	...
	public void UpdateHoverXY(int X, int Y)
	{
		...
		FindChild("SelectionHeader/PositionText").GetComponent<Text>().text = $"Mouse Position: {X}, {Y}";
	}
	...
	public void UpdateSelectedXY()
	{
		...
		{
			FindChild("SelectionHeader/SelectedText").GetComponent<Text>().text = $"Selected Cell: none";
			return;
		}
		if (SelectedRegion.size == Vector2.zero)
		{
			FindChild("SelectionHeader/SelectedText").GetComponent<Text>().text = $"Selected Cell: {SelectedRegion.x}, {SelectedRegion.y}";
			return;
		}
		FindChild("SelectionHeader/SelectedText").GetComponent<Text>().text = $"Selected Cell: {SelectedRegion.xMin}, {SelectedRegion.yMin} - {SelectedRegion.xMax}, {SelectedRegion.yMax}";
	}
	...
	public bool OnDragMove(Vector2 FingerPos, Vector2 Delta, PointerEventData.InputButton Button)
	{
		...
		{
			GameManager.MainCamera.GetComponent<LetterboxCamera>().SetPositionImmediately(GameManager.MainCamera.GetComponent<Camera>().transform.localPosition -= new Vector3(Delta.x, Delta.y, 0f));
		}
		return true;
	}
	...
	public IEnumerator _ReloadBlueprints()
	{
		...
		GetChildComponent<Text>("Working/Text").text = "Reloading ObjectBlueprints.xml\nMay take a few seconds...";
		...
		GetChildComponent<Text>("Working/Text").text = "Working...";
		yield return new WaitForEndOfFrame();
		yield return new WaitForEndOfFrame();
	}
	...
	public void NewMap()
	{
		...
		RedoStack = new Stack<MapEditorAction>();
		FileName = null;
		GetChild("Title").rootObject.GetComponent<Text>().text = "Map Editor - Untitled Map";
		_HasRendered = false;
		if (FilterUsed)
		{
			UpdateFilter();
	...
	public void Save()
	{
		...
		FileDialog fileDialog = DialogManager.createFileDialog();
		fileDialog.show(FileDialogMode.Save, "Save Map", "*.rpm", delegate(string s)
		{
			if (s != null)
			{
				FileName = s;
		...
		fileDialog.setPath(GetDefaultPath());
	}

	public void SaveAs()
	{
		FileDialog fileDialog = DialogManager.createFileDialog();
		fileDialog.show(FileDialogMode.Save, "Save Map As", "*.rpm", delegate(string s)
		{
			if (s != null)
			{
				FileName = s;
	...
	{
		FileDialog fileDialog = DialogManager.createFileDialog();
		fileDialog.show(FileDialogMode.Load, "Load Map", "*.rpm", delegate(string s)
		{
			if (s != null)
			{
				LoadMap(s);
	...
	public bool OnCommand(string Command, int nFinger = 0)
	{
		...
		{
			DataFilter = GetChild("InputField").rootObject.GetComponent<InputField>().text;
			GetChild("InputField").rootObject.GetComponent<InputField>().Select();
		}
		if (Command == "SaveAs")
		{
			SaveAs();
		...
		if (Command.StartsWith("Tile:"))
		{
			if (nFinger == 0)
			{
				if (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt))
				{
					if (Map != null)
					{
						string text2 = Command.Split(':')[1];
						int x = Convert.ToInt32(text2.Split(',')[0]);
						int y = Convert.ToInt32(text2.Split(',')[1]);
						if (Map.Cells[x, y].Objects.Count > 0)
						{
				...
				else if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl))
				{
					...
					int num3 = Convert.ToInt32(text3.Split(',')[0]);
					int num4 = Convert.ToInt32(text3.Split(',')[1]);
					if (Map != null && IsSingleObjectBrush())
					{
				...
				else if (Input.GetKey(KeyCode.LeftShift))
				{
					if (Map != null && IsSingleObjectBrush())
					{
						string text4 = Command.Split(':')[1];
						int x2 = Convert.ToInt32(text4.Split(',')[0]);
						int y2 = Convert.ToInt32(text4.Split(',')[1]);
				...
				{
					string text5 = Command.Split(':')[1];
					int num5 = Convert.ToInt32(text5.Split(',')[0]);
					int num6 = Convert.ToInt32(text5.Split(',')[1]);
			...
			if (nFinger == 1)
			{
				string text6 = Command.Split(':')[1];
				int x3 = Convert.ToInt32(text6.Split(',')[0]);
				int y3 = Convert.ToInt32(text6.Split(',')[1]);
